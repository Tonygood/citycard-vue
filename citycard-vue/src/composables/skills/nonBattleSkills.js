...
export function useNonBattleSkills() {
  ...
  function executeDianCiGanYing(caster, target) {
    ...
    // 公开日志：不显示城市名（施法者也只看到这条）
    gameStore.addLog(`${caster.name}对${target.name}使用了电磁感应`)
  ...
  function executeBanyunJiubingPutong(caster, selfCity) {
    ...
    addSkillUsageLog(
      ...
      `你使用搬运救兵·普通，从${provinceName}招募了${addedCities.map(c => `${c.name}(${c.hp})`).join('、')}`
    )
  ...
  function executeChenqibubeiZhiding(caster, target, targetCity) {
    ...
    // 检查谨慎交换集合
    if (gameStore.isInCautiousSet(target.name, cityName)) {
      return { success: false, message: '该城市在谨慎交换集合中，无法抢夺' }
    }
