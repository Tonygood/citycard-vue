...
const animationShownForRound = ref(-1)  // 已显示动画的回合号（防止重复显示）
const isFirstPlayerInRoom = ref(false)  // 当前玩家是否为房间第一个玩家
const confirmedBattleCitiesHpSnapshot = ref({})  // 确认出战时的城市HP快照，用于检测等待期间HP变化
  ...
  if (roomData) {
    ...
    if (playerData) {
      ...
      if (existingGameState && existingGameState.playerStates) {
        ...
        if (hasActiveGame) {
          console.log('[PlayerMode] handleAllReady - 检测到游戏已在进行中（mid-game reconnection），跳过中心城市选择')
          currentPlayer.value = JSON.parse(JSON.stringify(playerData))
          gameMode.value = roomData.mode || '2P'
...
async function handleMidGameReconnect({ name }) {
  console.log('[PlayerMode] Mid-game reconnect for:', name)
  const roomData = await getRoomData(currentRoomId.value)
  if (!roomData) return
...
async function handleSkillSelected(skillData) {
  ...
  if (skillData.result && skillData.result.success) {
    ...
    if (skillData.playerStatesSnapshot && Object.keys(skillData.playerStatesSnapshot).length > 0) {
      ...
      console.log('[PlayerMode] 已从快照恢复 gameStore.playerStates')
    }
    ...
    try {
      ...
      if (gameLogic.isGameOver.value) {
        console.log('[PlayerMode] 非战斗技能导致游戏结束！')
        ...
        console.log('[PlayerMode] 游戏结束状态已同步到Firebase')
...
async function handleDeploymentConfirmed({ cities, skill, skillData, activatedCitySkills }) {
  ...
  if (currentRoomId.value && currentPlayer.value) {
    ...
    // 保存确认出战城市的HP快照（用于等待期间检测对手技能导致的HP变化）
    confirmedBattleCitiesHpSnapshot.value = {}
    cities.forEach(cityName => {
      const city = currentPlayer.value.cities[cityName]
      if (city) {
        confirmedBattleCitiesHpSnapshot.value[cityName] = city.currentHp ?? city.hp
      }
    })
...
function startRoomDataListener() {
  ...
  roomDataListener = startRoomListener(currentRoomId.value, async (data) => {
    ...
    if (data.gameState) {
      ...
      // 检测等待期间对手技能导致已确认出战城市HP变化或被删除
      // 仅在等待对手确认时检测（!allDeployed），双方都已确认则直接进入战斗
      if (currentStep.value === 'game' &&
          !allDeployed &&
          Object.keys(confirmedBattleCitiesHpSnapshot.value).length > 0 &&
          !data.gameState.battleProcessed) {
        const myPlayerData = data.players.find(p => p.name === currentPlayer.value.name)
        if (myPlayerData?.cities) {
          let battleCitiesAffected = false
          for (const [cityName, snapshotHp] of Object.entries(confirmedBattleCitiesHpSnapshot.value)) {
            const city = myPlayerData.cities[cityName]
            if (!city) {
              battleCitiesAffected = true
              break
            }
          ...
          if (battleCitiesAffected) {
            currentPlayer.value.cities = JSON.parse(JSON.stringify(myPlayerData.cities))
            try {
              const { getDatabase, ref: dbRef, update } = await import('firebase/database')
              const db = getDatabase()
              const clearUpdates = {}
              clearUpdates[`rooms/${currentRoomId.value}/gameState/playerStates/${currentPlayer.value.name}/currentBattleCities`] = []
              clearUpdates[`rooms/${currentRoomId.value}/gameState/playerStates/${currentPlayer.value.name}/battleGoldSkill`] = null
              clearUpdates[`rooms/${currentRoomId.value}/gameState/playerStates/${currentPlayer.value.name}/battleGoldSkillData`] = null
              clearUpdates[`rooms/${currentRoomId.value}/gameState/playerStates/${currentPlayer.value.name}/activatedCitySkills`] = null
              await update(dbRef(db), clearUpdates)
            } catch (e) {
              console.error('[PlayerMode] 清除出战确认失败:', e)
            }
            confirmedBattleCitiesHpSnapshot.value = {}
            battleRetryCount.value = 0
            currentStep.value = 'city-deployment'
      ...
      if (allDeployed && !data.gameState.battleProcessed && !data.gameState.battleLock && !isBattleProcessing.value) {
        ...
        if (!data.players || data.players.length < expectedPlayers) {
          ...
          data.players?.forEach(p => normalizePlayerCities(p))
          console.log(`[PlayerMode] ✅ 重新获取数据成功，玩家数量: ${data.players.length}`)
        }

        // 所有玩家已部署，清除HP快照（不再需要检测变化）
        confirmedBattleCitiesHpSnapshot.value = {}
        ...
        // 避免仅依赖第一个玩家处理导致的卡死问题
          ...
          console.log(`[PlayerMode] 玩家${currentPlayer.value.name}尝试处理战斗 (${triggerTimestamp}), 重试次数: ${battleRetryCount.value}`)
      ...
      if (data.gameState.battleProcessed &&
        ...
        // 同时检查 animationShownForRound 防止重复显示
...
async function handleBattleAnimationComplete() {
  ...
  if (roomData?.gameState?.gameOver) {
    console.log('[PlayerMode] 动画后检测到游戏结束，获胜者:', roomData.gameState.winner)
...
async function processBattle(roomData) {
  ...
  console.log('[PlayerMode] 战斗前玩家数量:', roomData.players?.length)
  ...
  if (!roomData.players || roomData.players.length < expectedPlayers) {
    ...
    if (freshData && freshData.players && freshData.players.length >= expectedPlayers) {
      ...
      console.log(`[PlayerMode] ✅ processBattle: 从Firebase重新获取玩家数据成功，玩家数量: ${roomData.players.length}`)
    } else {
    ...
    if (state0 && state1) {
      ...
      state0.currentBattleCities?.forEach(card => {
        ...
        if (gameStore.purpleChamber && gameStore.purpleChamber[player0.name] === cityName) {
          console.log(`[PlayerMode] 跳过标记player0的${cityName}为已知（生于紫室保护）`)
          return
        }
      ...
      state1.currentBattleCities?.forEach(card => {
        ...
        if (gameStore.purpleChamber && gameStore.purpleChamber[player1.name] === cityName) {
          console.log(`[PlayerMode] 跳过标记player1的${cityName}为已知（生于紫室保护）`)
          return
        }
  ...
  console.log('[PlayerMode] roomData.players.length:', roomData.players?.length)
...
function restartGame() {
  ...
  isBattleProcessing.value = false
  battleRetryCount.value = 0
  animationShownForRound.value = -1
