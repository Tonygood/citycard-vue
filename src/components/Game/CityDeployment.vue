<template>
  ...
  <div class="deployment-with-log-layout">
    ...
    <div class="deployment-main-area">
      ...
      <ActiveSkillsPanel :current-player="currentPlayer" />
    </div>
...
import ActiveSkillsPanel from '../Skills/ActiveSkillsPanel.vue'
...
// 诊断日志：检查组件接收到的数据
debugLog('[CityDeployment] ===== 组件数据诊断 =====')
debugLog('[CityDeployment] currentPlayer.name:', props.currentPlayer?.name)
debugLog('[CityDeployment] currentPlayer.cities 数量:', Object.keys(props.currentPlayer?.cities || {}).length)
debugLog('[CityDeployment] currentPlayer.cities 详情:')
Object.entries(props.currentPlayer?.cities || {}).forEach(([cityName, city]) => {
  debugLog(`  [${cityName}] ${city.name} (HP: ${city.currentHp ?? city.hp})`)
})
debugLog('[CityDeployment] ===========================')
...
function handleSkillUsed(data) {
  debugLog('[CityDeployment] 技能使用成功', data)
  ...
  debugLog('[CityDeployment] emit skill-used 事件给父组件', data)
  ...
  if (isBattleSkill) {
    ...
    debugLog('[CityDeployment] 保存战斗技能:', usedBattleSkill.value, usedBattleSkillData.value)
  }
  if (skillName === '按兵不动') {
    debugLog('[CityDeployment] 按兵不动生效，自动确认部署（不出战任何城市）')
...
async function handleSkillFailed(data) {
  debugLog('[CityDeployment] 技能使用失败', data)
...
function confirmDeployment() {
  ...
  // 添加诊断日志
  debugLog('[CityDeployment] 确认部署')
  debugLog('[CityDeployment] selectedCities:', selectedCities.value)
  selectedCities.value.forEach(cityName => {
    const city = props.currentPlayer.cities[cityName]
    debugLog(`[CityDeployment] 选中城市 cityName=${cityName}, name=${city?.name}`)
  })
}
</script>
...
@media (max-width: 768px) {
  ...
  .confirm-btn,
  .cancel-btn,
  .skill-detail-modal .skill-detail-content,
  .forced-deployment-notice,
  .an-bing-bu-dong-notice {
