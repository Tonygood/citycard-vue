...
const combinedLogs = computed(() => {
  ...
  // 添加公共日志（排除不属于当前玩家的私密日志）
  if (gameStore.logs && Array.isArray(gameStore.logs)) {
    gameStore.logs.forEach(log => {
      if (log.isPrivate && log.visibleTo) {
        // 私密日志：仅当当前玩家在visibleTo列表中才显示
        if (props.currentPlayerName && log.visibleTo.includes(props.currentPlayerName)) {
          allLogs.push({ ...log, isPrivate: true })
        }
      } else {
        // 公共日志
        allLogs.push({ ...log, isPrivate: false })
      }
    })
  }
