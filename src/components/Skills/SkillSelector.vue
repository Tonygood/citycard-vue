          ...
          <div
            ...
            :class="[
              'mini-city-card',
              {
                ...
                'disabled': item.city.isAlive === false && selectedSkill?.name !== '以礼来降',
                'dead': item.city.isAlive === false && selectedSkill?.name === '以礼来降'
              }
            ]"
...
import { SKILL_COSTS, getActualSkillCost } from '../../constants/skillCosts'
import {
  BATTLE_SKILLS,
  NON_BATTLE_SKILLS,
...
const SKILL_METADATA = {
  ...
  '快速治疗': { emoji: '💊', category: 'protection', description: '城市增加15000HP', requiresSelfCity: true },
  ...
  '时来运转': { emoji: '🎲', category: 'resource', description: '随机交换双方3座城市', requiresTarget: true, showKnownCities: false },
  '实力增强': { emoji: '💪', category: 'buff', description: 'HP翻倍，上限50000', requiresTarget: false, requiresSelfCity: true },
  '人质交换': { emoji: '⛓️', category: 'control', description: '交换己方第4名和对手玩家第3名的城市', requiresTarget: true, showKnownCities: false  },
  ...
  '搬运救兵': { emoji: '🚚', category: 'resource', description: '获得同省2座随机城市', requiresSelfCity: true },
  ...
  '趁其不备': { emoji: '🎲', category: 'damage', description: '抢夺对手随机一座城市', requiresTarget: true, showKnownCities: false },
  ...
  '强制搬运': { emoji: '🚚', category: 'control', description: '强制对手使用搬运救兵技能，且本回合必须出战这几座城市', requiresTarget: true },
  ...
  '以礼来降': { emoji: '🏳️', category: 'resource', description: '抢夺对手一座城市，并随机获得一座城市', requiresTarget: true, requiresTargetCity: true },
...
const allSkills = computed(() => {
  ...
  if (props.skillType === 'all' || props.skillType === 'nonBattle') {
    NON_BATTLE_SKILLS.forEach(skillName => {
      ...
      // 无懈可击已重构为实时拦截系统，不再作为主动技能出现在选择器中
      if (skillName === '无懈可击') return
      // 李代桃僵已重构为被动触发（通过时来运转/人质交换弹窗），不再作为主动技能
      if (skillName === '李代桃僵') return
...
function getSkillCost(skillName) {
  // 显示实际花费（考虑釜底抽薪等费用增加效果）
  return getActualSkillCost(skillName, props.currentPlayer, gameStore)
}
...
function handleCityClick(cityNameOrIdx, city, type) {
  ...
  } else if (selectedSkill.value && selectedSkill.value.name === '以礼来降') {
    // 以礼来降：允许选择已阵亡的城市（不做阵亡限制）
  } else {
    // 其他技能：不能点击已阵亡的城市
    if (isCityDead) return
  }
...
function getBannableSkills() {
  const SKILL_COST_MAP = {
    ...
    '趁其不备': 10, '当机立断': 10,
    '搬运救兵': 11, '无懈可击': 11, '副中心制': 11, '言听计从': 7,
    '中庸之道': 12, '步步高升': 12,
...
function getTargetCities() {
  ...
  if (typeof player.cities === 'object' && !Array.isArray(player.cities)) {
    ...
    Object.entries(player.cities).forEach(([cityName, city]) => {
      ...
      // 对于以礼来降，只过滤掉中心城市和钢铁城市
      if (selectedSkill.value?.name === '以礼来降') {
        if (cityName === centerCityName) return
        if (gameStore.ironCities[player.name] && gameStore.ironCities[player.name][cityName] > 0) return
      }
    ...
    player.cities
      ...
      .forEach(item => {
        ...
        if (selectedSkill.value?.name === '以礼来降') {
          ...
          if (gameStore.ironCities[player.name] && gameStore.ironCities[player.name][item.city.name] > 0) return
        }
...
const SKILL_EXECUTOR_MAP = {
  ...
  '先声夺人': () => {
    // selfCity.value 已经是城市名称字符串，直接传递
    return nonBattleSkills.executeXianShengDuoRen(getCasterPlayer(), getTargetPlayer(), { casterCityName: selfCity.value })
  },
  ...
  '博学多才': () => nonBattleSkills.executeBoXueDuoCai(getCasterPlayer(), selfCity.value, -1),
  ...
  '搬运救兵': () => nonBattleSkills.executeBanyunJiubingPutong(getCasterPlayer(), getSelfCityObject()),
  '趁其不备': () => nonBattleSkills.executeChenqibubeiSuiji(getCasterPlayer(), getTargetPlayer()),
...
function startBxdcQuiz() {
  ...
  // 确定题库来源：城市专属 → 省份 → DEFAULT（考虑拔旗易帜）
  let questionSource = null
  if (hasCityQuestions(cityName)) {
    // 城市有专属题目
    questionSource = CITY_QUESTIONS[cityName]
  } else {
    // 没有专属题目，尝试使用省份题目（考虑拔旗易帜changeFlagMark）
    let provinceName = null
    if (gameStore.changeFlagMark &&
        gameStore.changeFlagMark[props.currentPlayer.name] &&
        gameStore.changeFlagMark[props.currentPlayer.name][cityName]) {
      provinceName = gameStore.changeFlagMark[props.currentPlayer.name][cityName].newProvince
    } else {
...
function finishBxdcQuiz(correctCount) {
  ...
  debugLog('[SkillSelector] 博学多才答题完成，答对:', correctCount, '结果:', result)
  ...
  if (result.success) {
    ...
    // 关键修复：保存技能执行后的gameStore快照（同executeSkill）
    ...
    const skillUsageSnapshot = JSON.parse(JSON.stringify(gameStore.skillUsageTracking || {}))
    const cooldownsSnapshot = JSON.parse(JSON.stringify(gameStore.cooldowns || {}))
    const roundGoldSpentSnapshot = JSON.parse(JSON.stringify(gameStore.roundGoldSpent || {}))
    const bannedSkillsSnapshot = JSON.parse(JSON.stringify(gameStore.bannedSkills || {}))
    pendingSkillEmit.value = {
      ...
      skillUsageSnapshot,
      cooldownsSnapshot,
      roundGoldSpentSnapshot,
      bannedSkillsSnapshot
    }
...
function executeSkill() {
  ...
  // 单回合技能总花费限制：总花费需小于20金币（单用20金币及以上技能除外）
  const skillCost = getActualSkillCost(skill.name, props.currentPlayer, gameStore)
  const spentThisRound = gameStore.getRoundGoldSpent(props.currentPlayer.name)
  if (spentThisRound >= 20) {
    showAlert('本回合已使用了20金币及以上技能，无法使用其他技能')
    return
  }
  if (skillCost >= 20) {
    if (spentThisRound > 0) {
      showAlert('本回合已使用了其他金币技能，无法使用20金币及以上技能')
      return
    }
  } else if (skillCost > 0) {
    if (spentThisRound + skillCost >= 20) {
      showAlert(`本回合已花费${spentThisRound}金币，再花费${skillCost}金币将达到20金币上限`)
      return
    }
  }
  ...
  // 先声夺人前置检查：对手是否有可交换城市
  if (skill.name === '先声夺人') {
    const target = gameStore.players.find(p => p.name === targetPlayer.value)
    if (target) {
      const hasEligible = Object.entries(target.cities).some(([cityName, city]) => {
        if (city.isAlive === false) return false
        if (gameStore.isInCautiousSet(target.name, cityName)) return false
        if (city.isCenter) return false
        if (gameStore.anchored?.[target.name]?.[cityName]) return false
        if (gameStore.hasIronShield(target.name, cityName)) return false
        if (gameStore.hasProtection(target.name, cityName)) return false
        return true
      })
      if (!hasEligible) {
        showAlert('对手没有可交换的城市，技能使用失败')
        return
      }
    }
  }

  // 博学多才：先扣金币走拦截流程，答题在拦截通过后进行（在PlayerModeOnline中处理）

  let result
  // 记录技能执行前的日志数量，用于拦截流程回滚本地日志
  const logsLengthBeforeExecution = gameStore.logs.length
  const privateLogsLengthBeforeExecution = (gameStore.playerPrivateLogs?.[props.currentPlayer.name] || []).length
  ...
  try {
    ...
    if (result.success) {
      ...
      // Store emit payload and show animation
      // 关键修复：保存技能执行后的gameStore快照
      // 动画播放期间Firebase监听器可能用旧数据覆盖gameStore状态
      ...
      pendingSkillEmit.value = {
        ...
        selfCityName: selfCity.value,
        selectedSelfCities: selectedSelfCities.value.length > 0 ? [...selectedSelfCities.value] : undefined,
        selectedTargetCities: selectedTargetCities.value.length > 0 ? [...selectedTargetCities.value] : undefined,
        selectedSkillName: selectedSkillName.value || undefined,
        amount: amount.value,
        playersSnapshot,
        playerStatesSnapshot,
        skillUsageSnapshot,
        ...
        roundGoldSpentSnapshot,
        bannedSkillsSnapshot,
        logsLengthBeforeExecution,
        privateLogsLengthBeforeExecution
      }
      skillAnimationConfig.value = getSkillAnimation(skill.name)
      showSkillAnimation.value = true
    } else {
      debugLog('[SkillSelector] 技能执行失败，发出 skill-failed 事件:', { skill: skill.name, result })
      emit('skill-failed', { skill: skill.name, result })
    }
  } catch (error) {
