...
function swapCityStates(gameStore, player1Name, city1Name, player2Name, city2Name) {
  ...
  if (gameStore.purpleChamber[player1Name] === city1Name) {
    ...
    debugLog(`[先声夺人] 更新生于紫室: ${player1Name} ${city1Name} -> ${city2Name}`)
  }
  if (gameStore.purpleChamber[player2Name] === city2Name) {
    gameStore.purpleChamber[player2Name] = city1Name
    debugLog(`[先声夺人] 更新生于紫室: ${player2Name} ${city2Name} -> ${city1Name}`)
  }
  ...
  if (gameStore.cautiousExchange[player1Name]?.has(city1Name)) {
    ...
    debugLog(`[先声夺人] 更新cautiousExchange: ${player1Name} ${city1Name} -> ${city2Name}`)
  }
  if (gameStore.cautiousExchange[player2Name]?.has(city2Name)) {
    ...
    debugLog(`[先声夺人] 更新cautiousExchange: ${player2Name} ${city2Name} -> ${city1Name}`)
  }
  ...
  if (gameStore.cautionSet[player1Name]?.has(city1Name)) {
    ...
    debugLog(`[先声夺人] 更新cautionSet: ${player1Name} ${city1Name} -> ${city2Name}`)
  }
  if (gameStore.cautionSet[player2Name]?.has(city2Name)) {
    ...
    debugLog(`[先声夺人] 更新cautionSet: ${player2Name} ${city2Name} -> ${city1Name}`)
  }

  // 注意：其他所有状态（anchored, ironCities, protections等）都使用
  // [playerName][cityName] 作为键，城市交换时会自动跟随城市名称，无需手动交换
  debugLog('[先声夺人] 城市状态交换完成（其他状态自动跟随cityName）')
}
...
export function useNonBattleSkills() {
  ...
  function executeWuZhiWuWei(caster, target) {
    ...
    // 排除副中心城市（副中心不会被自动选为自毁城市，战略转移到中心后可用）
    const selectableCities = aliveCities.filter(c => !gameStore.isSubCenter(caster.name, c.name))
    if (selectableCities.length === 0) {
      return { success: false, message: '没有可用城市（副中心城市不可自毁）' }
    }

    // 找到血量最低的城市
    const lowestHpCity = selectableCities.reduce((min, city) =>
  ...
  function executeKuaiSuZhiLiao(caster, selfCity) {
    ...
    // maxHp为城市当前的基础HP（selfCity.hp），会随一落千丈等技能变化
    const maxHp = selfCity.hp
    ...
    selfCity.currentHp = Math.min(currentHp + 15000, maxHp)
    const healed = selfCity.currentHp - before
    ...
    addSkillUsageLog(
      ...
      '快速治疗',
      `${caster.name}使用了快速治疗`,
      `你对${selfCity.name}使用了快速治疗，HP+${healed}（${Math.floor(before)} → ${Math.floor(selfCity.currentHp)}）`
    )

    return {
      success: true,
      message: `${selfCity.name} HP+${healed}`
    }
  }
  ...
  function executeJieShiHuanHun(caster, selfCity) {
    ...
    // 清除疲劳指数（修复：必须重置player.streaks，这是applyFatigueReduction实际读取的属性）
    const cityName = selfCity.name
    if (cityName) {
      // 重置player对象上的streaks（fatigue系统使用的实际属性）
      if (!caster.streaks) caster.streaks = {}
      caster.streaks[cityName] = 0
      // 同时重置gameStore.fatigueStreaks（兼容旧代码）
      ...
      // 标记为本轮复活的城市（applyFatigueReduction会跳过本轮复活的城市）
      if (!gameStore.revivedCities[caster.name]) {
        gameStore.revivedCities[caster.name] = {}
      }
      gameStore.revivedCities[caster.name][cityName] = true
    }

    // 清除生于紫室状态（复活后不保留）
    if (gameStore.purpleChamber && gameStore.purpleChamber[caster.name] === cityName) {
      delete gameStore.purpleChamber[caster.name]
    }
  ...
  function executeShiQiDaZhen(caster) {
    ...
    // 恢复所有存活且当前HP低于初始HP的城市至初始HP
    let count = 0
    Object.values(caster.cities).forEach(city => {
      if (city && city.isAlive !== false && city.currentHp < city.hp) {
  ...
  function executeQingChuJiaCheng(caster, target, targetCity) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, cityName)) {
      addSkillUsageLog(gameStore, caster.name, '清除加成',
        `${caster.name}使用了清除加成，但目标是副中心城市，技能无效`,
        `${caster.name}使用了清除加成`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }

    // 检查钢铁城市
    const hasIron = gameStore.hasIronShield(target.name, cityName)
    if (hasIron) {
      const ironLevel = gameStore.ironCities[target.name][cityName]
      if (ironLevel > 1) {
        // 第一次清除：只减少钢铁层数，不做其他任何事
        gameStore.ironCities[target.name][cityName]--
        addSkillUsageLog(gameStore, caster.name, '清除加成',
          `${caster.name}对${target.name}的${cityName}使用清除加成，钢铁护盾减一层`,
          `${caster.name}使用了清除加成`)
        return { success: true, message: '钢铁城市护盾减少一层' }
      } else {
        // 第二次清除：移除钢铁护盾，然后继续执行完整清除
        delete gameStore.ironCities[target.name][cityName]
        addSkillEffectLog(gameStore, `${target.name}的${cityName}钢铁护盾被完全清除`)
        // 继续往下执行完整清除逻辑
      }
    }

    // 消耗城市保护（非钢铁）
    if (!hasIron && gameStore.hasProtection(target.name, cityName)) {
      gameStore.consumeProtection(target.name, cityName)
  ...
  /**
   * 时来运转 - 准备阶段：随机选择3张卡牌（不执行实际交换）
   * 返回 isPending: true 表示需要弹窗确认
   */
  function executeShiLaiYunZhuan(caster, target) {
    ...
    const casterCities = casterPicks.map(p => p.cityName)
    const targetCities = targetPicks.map(p => p.cityName)
    ...
    return {
      ...
      casterCities,
      targetCities,
      message: `时来运转准备完成，等待对手响应`
    }
  }

  /**
   * 时来运转 - 执行阶段：实际执行城市交换
   * @param {string} casterName - 施法者名称
   * @param {string} targetName - 目标名称
   * @param {string[]} casterCities - 施法者参与交换的城市名称数组
   * @param {string[]} targetCities - 目标参与交换的城市名称数组
   */
  function executeFortuneSwap(casterName, targetName, casterCities, targetCities) {
    const caster = gameStore.players.find(p => p.name === casterName)
    const target = gameStore.players.find(p => p.name === targetName)
    ...
    for (let k = 0; k < minLen; k++) {
      ...
      const casterCityObj = caster.cities[casterCityKey]
      const targetCityObj = target.cities[targetCityKey]

      // 正确交换name-keyed城市
      delete caster.cities[casterCityKey]
      delete target.cities[targetCityKey]

      caster.cities[targetCityKey] = targetCityObj
      target.cities[casterCityKey] = casterCityObj

      // 同步交换initialCities
      if (gameStore.initialCities[caster.name] && gameStore.initialCities[target.name]) {
        const casterInitial = gameStore.initialCities[caster.name][casterCityKey]
        const targetInitial = gameStore.initialCities[target.name][targetCityKey]

        delete gameStore.initialCities[caster.name][casterCityKey]
        delete gameStore.initialCities[target.name][targetCityKey]

        if (targetInitial) gameStore.initialCities[caster.name][targetCityKey] = targetInitial
        if (casterInitial) gameStore.initialCities[target.name][casterCityKey] = casterInitial
      }

      // 标记城市为已知
      gameStore.setCityKnown(caster.name, targetCityKey, target.name)
      gameStore.setCityKnown(target.name, casterCityKey, caster.name)

      swappedPairs.push({
        casterCity: casterCityObj.name,
        targetCity: targetCityObj.name
      })
    }
    ...
    for (let k = 0; k < minLen; k++) {
      ...
      const casterStreak = caster.streaks[casterCityKey] || 0
      const targetStreak = target.streaks[targetCityKey] || 0

      delete caster.streaks[casterCityKey]
      delete target.streaks[targetCityKey]

      caster.streaks[targetCityKey] = targetStreak
      target.streaks[casterCityKey] = casterStreak
    }
    ...
    for (let k = 0; k < minLen; k++) {
      ...
      if (targetMark) {
        gameStore.changeFlagMark[caster.name][targetCityKey] = { ...targetMark }
      }
      if (casterMark) {
        gameStore.changeFlagMark[target.name][casterCityKey] = { ...casterMark }
      }
    }
  ...
  function executeXianShengDuoRen(caster, target, params = {}) {
    ...
    debugLog('[先声夺人] ===== 创建交换请求 =====')
    debugLog('[先声夺人] 返回的 swap:', swap)
    debugLog('[先声夺人] swap.initiatorCityName:', swap?.initiatorCityName)
    debugLog('[先声夺人] 可交换城市列表:')
    debugLog(`[先声夺人]   ${caster.name}: [${eligibleCaster.join(', ')}]`)
    debugLog(`[先声夺人]   ${target.name}: [${eligibleTarget.join(', ')}]`)
    debugLog('[先声夺人] ===============================')
    ...
    debugLog(`[先声夺人] ${caster.name} 发起请求`)
    debugLog(`[先声夺人] 待处理请求ID: ${swap.id}`)
    debugLog(`[先声夺人] ${caster.name}选择: ${casterCityName}`)
    debugLog(`[先声夺人] 等待${target.name}响应...`)
  ...
  function acceptPreemptiveStrike(swapId, targetCityName) {
    ...
    if (gameStore.disguisedCities[initiator.name] &&
      ...
      debugLog(`[先声夺人] 清除${initiator.name}的伪装状态`)
    }
    if (gameStore.disguisedCities[targetPlayer.name] &&
      ...
      disguiseCleared = true
      debugLog(`[先声夺人] 清除${targetPlayer.name}的伪装状态`)
    }
    ...
    debugLog(`[先声夺人] ===== 交换完成总结 =====`)
    debugLog(`[先声夺人] ${initiator.name}[${initiatorCityName}] ⇄ ${targetPlayer.name}[${targetCityNameForLog}]`)
    swapDetails.forEach(detail => debugLog(`[先声夺人] - ${detail}`))
    debugLog(`[先声夺人] ============================`)
  ...
  function rejectPreemptiveStrike(swapId) {
    ...
    debugLog(`[先声夺人] ${swap.targetName}使用无懈可击拒绝交换，花费${SKILL_COSTS['无懈可击']}金币，剩余${targetPlayer.gold}金币`)
    ...
    debugLog(`[先声夺人] 退还${swap.initiatorName}的使用次数`)

    // 清除冷却时间
    gameStore.clearSkillCooldown(swap.initiatorName, '先声夺人')
    debugLog(`[先声夺人] 清除${swap.initiatorName}的冷却时间`)

    // 双日志：记录拒绝（使用无懈可击）
    addSkillEffectLog(
      gameStore,
      `(先声夺人) ${swap.targetName}使用无懈可击拒绝了${swap.initiatorName}的交换请求（花费${SKILL_COSTS['无懈可击']}金币）`
    )

    debugLog(`[先声夺人] ${swap.targetName}拒绝了交换请求`)

    return {
      success: true,
      message: `已使用无懈可击拒绝交换请求（花费${SKILL_COSTS['无懈可击']}金币，剩余${targetPlayer.gold}金币）`
    }
  }
  ...
  function executeDingHaiShenZhen(caster, selfCity) {
    ...
    return {
      success: true,
      message: `${caster.name}使用了定海神针，该城市10回合内不会被交换`,
      hidePublicLog: true
    }
  }
  ...
  function executeGouYanCanChuan(caster) {
    ...
    // 双日志：公开只显示使用了技能，获得的城市信息为私密
    addSkillUsageLog(
      gameStore,
      caster.name,
      '苟延残喘',
      `${caster.name}使用了苟延残喘`,
      `你使用苟延残喘，获得了${addedCities.map(c => `${c.name}(${c.hp})`).join('、')}`
    )
  ...
  function executeHaoGaoWuYuan(caster, selfCity) {
    ...
    if (caster.centerCityName === oldCityKey) {
      ...
      debugLog(`[点石成金] 中心城市从 ${oldCityKey} 转移到 ${newCity.name}`)
    }

    // 保留步步高升状态（更新originalHp为新城市HP）
    if (gameStore.bbgs && gameStore.bbgs[caster.name] && gameStore.bbgs[caster.name][oldCityKey]) {
      gameStore.bbgs[caster.name][newCity.name] = gameStore.bbgs[caster.name][oldCityKey]
      gameStore.bbgs[caster.name][newCity.name].originalHp = newCity.hp
      delete gameStore.bbgs[caster.name][oldCityKey]
    }
    ...
    if (gameStore.initialCities[caster.name]) {
      delete gameStore.initialCities[caster.name][oldCityKey]
      gameStore.initialCities[caster.name][newCity.name] = {
        name: newCity.name,
        hp: newCity.hp,
        currentHp: newCity.hp,
  ...
  function executePaoZhuanYinYu(caster) {
    ...
    addSkillUsageLog(
      ...
      caster.name,
      '抛砖引玉',
      `${caster.name}使用抛砖引玉，自毁${targetCity.name}(HP:${Math.floor(oldHp)})，获得${goldGain}金币`,
  ...
  function executeJinZhiNiuQu(caster, target, targetCity) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, cityName)) {
      addSkillUsageLog(gameStore, caster.name, '进制扭曲',
        `${caster.name}使用了进制扭曲，但目标是副中心城市，技能无效`,
        `${caster.name}使用了进制扭曲`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeYiLuoQianZhang(caster, target, targetCity) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, cityName)) {
      addSkillUsageLog(gameStore, caster.name, '一落千丈',
        `${caster.name}使用了一落千丈，但目标是副中心城市，技能无效`,
        `${caster.name}使用了一落千丈`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeLianXuDaJi(caster, target, cityNames) {
    ...
    for (const { city, cityName } of validCities) {
      if (gameStore.isSubCenter(target.name, cityName)) {
        addSkillUsageLog(gameStore, caster.name, '连续打击',
          `${caster.name}使用了连续打击，但目标${cityName}是副中心城市，技能无效`,
          `${caster.name}使用了连续打击`)
        return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
      }
    }
  ...
  function executeBoTaoXiongYong(caster, target) {
    ...
    // 第一部分：获取对手所有沿海城市
    const coastalCityNames = new Set([
      '上海市', '天津市', '香港特别行政区', '澳门特别行政区', '唐山市', '秦皇岛市', '沧州市',
      '大连市', '丹东市', '锦州市', '营口市', '盘锦市', '葫芦岛市', '南通市', '连云港市', '盐城市',
      '宁波市', '温州市', '嘉兴市', '台州市', '舟山市', '福州市', '莆田市',
      '泉州市', '厦门市', '漳州市', '宁德市', '青岛市', '东营市', '烟台市', '威海市', '日照市',
      '潍坊市', '滨州市', '德州市', '广州市', '深圳市', '珠海市', '汕头市', '湛江市', '江门市', '茂名市',
      '惠州市', '汕尾市', '阳江市', '东莞市', '中山市', '潮州市', '揭阳市', '北海市', '钦州市',
      '防城港市', '海口市', '三亚市', '三沙市', '琼海市', '文昌市', '万宁市', '澄迈县', '临高县',
      '东方市', '昌江县', '乐东县', '陵水县', '儋州市', '桃园市', '新北市', '基隆市', '台南市',
      '台中市', '新竹市', '高雄市', '彰化县', '新竹县', '澎湖县', '台东县', '宜兰县', '屏东县',
      '嘉义县', '云林县', '花莲县', '苗栗县'
    ])
    const coastalCities = []

    Object.entries(target.cities).forEach(([cityName, city]) => {
      if (city && city.isAlive !== false && coastalCityNames.has(city.name)) {
    ...
    for (const { city, cityName } of coastalCities) {
      // 跳过副中心城市（免疫效果）
      if (gameStore.isSubCenter(target.name, cityName)) continue
  ...
  function executeWanJianQiFa(caster, target, targetCity) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, targetCity.name)) {
      addSkillUsageLog(gameStore, caster.name, '万箭齐发',
        `${caster.name}使用了万箭齐发，但目标是副中心城市，技能无效`,
        `${caster.name}使用了万箭齐发`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeJiangWeiDaJi(caster, target, targetCity) {
    ...
    if (isMunicipality) {
      ...
      // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
      if (gameStore.isSubCenter(target.name, cityName)) {
        addSkillUsageLog(gameStore, caster.name, '降维打击',
          `${caster.name}使用了降维打击，但目标是副中心城市，技能无效`,
          `${caster.name}使用了降维打击`)
        return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
      }
      ...
      // 保留步步高升状态（更新originalHp为澳门HP）
      if (gameStore.bbgs && gameStore.bbgs[target.name] && gameStore.bbgs[target.name][cityName]) {
        gameStore.bbgs[target.name]['澳门特别行政区'] = gameStore.bbgs[target.name][cityName]
        gameStore.bbgs[target.name]['澳门特别行政区'].originalHp = macau.hp
        delete gameStore.bbgs[target.name][cityName]
      }
    ...
    // 获取该城市所属省份（考虑拔旗易帜changeFlagMark）
    let province = gameStore.getProvinceOfCity(targetCity.name)
    if (gameStore.changeFlagMark &&
        gameStore.changeFlagMark[target.name] &&
        gameStore.changeFlagMark[target.name][targetCity.name]) {
      const newProvName = gameStore.changeFlagMark[target.name][targetCity.name].newProvince
      // 通过省份名称查找省份对象
      const provKey = Object.keys(PROVINCE_MAP).find(key => PROVINCE_MAP[key].name === newProvName)
      if (provKey) {
        province = PROVINCE_MAP[provKey]
      }
    }
    if (!province) {
    ...
    // 保留步步高升状态（更新originalHp为新城市HP）
    if (gameStore.bbgs && gameStore.bbgs[target.name] && gameStore.bbgs[target.name][targetCityKey]) {
      gameStore.bbgs[target.name][replacementCity.name] = gameStore.bbgs[target.name][targetCityKey]
      gameStore.bbgs[target.name][replacementCity.name].originalHp = replacementCity.hp
      delete gameStore.bbgs[target.name][targetCityKey]
    }
  ...
  function executeDingShiBaoPo(caster, target, targetCity) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, cityName)) {
      addSkillUsageLog(gameStore, caster.name, '定时爆破',
        `${caster.name}使用了定时爆破，但目标是副中心城市，技能无效`,
        `${caster.name}使用了定时爆破`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeYongJiuCuiHui(caster, target, targetCity) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, cityName)) {
      addSkillUsageLog(gameStore, caster.name, '灰飞烟灭',
        `${caster.name}使用了灰飞烟灭，但目标是副中心城市，技能无效`,
        `${caster.name}使用了灰飞烟灭`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
    ...
    // 步步高升触发：灰飞烟灭摧毁城市前，检查并触发步步高升召唤
    let bbgsSummoned = false
    if (gameStore.bbgs && gameStore.bbgs[target.name] && gameStore.bbgs[target.name][cityName]) {
      // 模拟城市阵亡以触发步步高升
      gameStore.handleBuBuGaoShengSummon(target, cityName, targetCity)
      bbgsSummoned = true
    }
    ...
    return {
      success: true,
      message: `${targetCity.name}已被灰飞烟灭${bbgsSummoned ? '（步步高升已触发召唤）' : ''}`
    }
  }
  ...
  function executeLianSuoFanYing(caster, target, targetCity) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, cityName)) {
      addSkillUsageLog(gameStore, caster.name, '连锁反应',
        `${caster.name}使用了连锁反应，但目标是副中心城市，技能无效`,
        `${caster.name}使用了连锁反应`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
    ...
    // 阶段2：目标已死，对其余所有存活城市造成1000溅射伤害
    const allCityNames = Object.keys(target.cities)
    const otherAliveCities = allCityNames
      .filter(cn => cn !== cityName && target.cities[cn] && target.cities[cn].isAlive !== false)

    if (otherAliveCities.length === 0) {
      addSkillEffectLog(gameStore, `连锁反应：目标城市阵亡，但对手无其他存活城市`)
      return {
        success: true,
        message: `${targetName}阵亡，但对手无其他存活城市`
      }
    }

    const deadFrom1000 = []
    otherAliveCities.forEach(aliveCityName => {
      const city = target.cities[aliveCityName]
      const cName = city.name

      // 跳过副中心城市（免疫连锁反应溅射）
      if (gameStore.isSubCenter(target.name, aliveCityName)) return

      // 检查并消耗保护罩/钢铁护盾
      if (gameStore.consumeProtection(target.name, aliveCityName)) {
      ...
      if (cityNewHp <= 0) {
        city.isAlive = false
        if (!gameStore.deadCities[target.name].includes(aliveCityName)) {
          gameStore.deadCities[target.name].push(aliveCityName)
        }
    ...
    if (deadFrom1000.length === 0) {
      addSkillEffectLog(gameStore, `连锁反应：目标城市阵亡，其余城市各受1000伤害`)
      return {
        success: true,
        message: `${targetName}阵亡，其余城市各受1000伤害`
      }
    }

    // 阶段3：有城市因1000伤害阵亡，对仍存活的城市额外造成500伤害
    const stillAlive = otherAliveCities.filter(cn => target.cities[cn].isAlive !== false)
    const deadFrom500 = []

    stillAlive.forEach(aliveCityName => {
      const city = target.cities[aliveCityName]
      const cName2 = city.name
      ...
      // 检查并消耗保护罩/钢铁护盾
      if (gameStore.consumeProtection(target.name, aliveCityName)) {
        addSkillEffectLog(gameStore, `${cName2}的护盾抵消了500伤害`)
        return
      }
  ...
  function executeZhaoXianNaShi(caster, target) {
    ...
    Object.entries(target.cities).forEach(([cityKey, city]) => {
      ...
      // 排除副中心城市（免疫效果）
      if (gameStore.isSubCenter(target.name, cityKey)) return
    ...
    if (targetCities.length === 0) {
      return {
        success: false,
        message: `${target.name}没有符合条件的城市（HP<${Math.floor(thresholdHp)}且非中心/钢铁/定海神针/副中心城市）`
      }
    }
  ...
  // 无懈可击已重构为实时拦截系统（在PlayerModeOnline.vue中实现）
  // executeWuXieKeJi 已移除
  ...
  function executeRenZhiJiaoHuan(caster, target) {
    ...
    // 获取所有存活城市，按当前HP排序（降序）
    const mySorted = Object.keys(caster.cities)
      .filter(cityName => {
        const city = caster.cities[cityName]
        return city && (city.currentHp ?? city.hp) > 0 && city.isAlive !== false
      })
      .map(cityName => ({
        cityName,
    ...
    const oppSorted = Object.keys(target.cities)
      .filter(cityName => {
        const city = target.cities[cityName]
        return city && (city.currentHp ?? city.hp) > 0 && city.isAlive !== false
      })
    ...
    if (mySorted.length < 4) {
      return {
        success: false,
        message: `你的存活城市不足4个（当前${mySorted.length}个）`
      }
    }

    if (oppSorted.length < 3) {
      return {
        success: false,
        message: `对方的存活城市不足3个（当前${oppSorted.length}个）`
      }
    }

    // 我方选择当前HP第4的城市
    const myCityName = mySorted[3].cityName

    // 对方选择当前HP第3的城市
    const oppCityName = oppSorted[2].cityName

    // 检查谨慎交换集合（定海神针、钢铁城市、中心城市等）
    if (gameStore.isInCautiousSet(caster.name, myCityName) || gameStore.isInCautiousSet(target.name, oppCityName)) {
      return {
        success: false,
        message: '可交换的城市位于谨慎交换集合，无法使用人质交换'
      }
    }

    // 金币检查和扣除（放在谨慎交换集合检查之后，确保技能可以成功再扣金币）
    ...
    return {
      success: true,
      isPending: true,
      casterCities: [myCityName],
      targetCities: [oppCityName],
      message: `人质交换准备完成，等待对手响应`
    }
  }
   ...
   * @param {string} casterName - 施法者名称
   * @param {string} targetName - 目标名称
   * @param {string} casterCityKey - 施法者参与交换的城市名称
   * @param {string} targetCityKey - 目标参与交换的城市名称
   */
  function executeHostageSwap(casterName, targetName, casterCityKey, targetCityKey) {
    const caster = gameStore.players.find(p => p.name === casterName)
    const target = gameStore.players.find(p => p.name === targetName)
    ...
    const myCityObj = caster.cities[casterCityKey]
    const oppCityObj = target.cities[targetCityKey]

    if (!myCityObj || !oppCityObj) {
      return { success: false, message: '交换城市不存在' }
    }
    ...
    caster.cities[targetCityKey] = oppCityObj
    target.cities[casterCityKey] = myCityObj

    // 同步交换initialCities
    if (gameStore.initialCities[caster.name] && gameStore.initialCities[target.name]) {
      const myInitial = gameStore.initialCities[caster.name][casterCityKey]
      const oppInitial = gameStore.initialCities[target.name][targetCityKey]

      delete gameStore.initialCities[caster.name][casterCityKey]
      delete gameStore.initialCities[target.name][targetCityKey]

      if (oppInitial) gameStore.initialCities[caster.name][targetCityKey] = oppInitial
      if (myInitial) gameStore.initialCities[target.name][casterCityKey] = myInitial
    }
    ...
    const myStreak = caster.streaks[casterCityKey] || 0
    const oppStreak = target.streaks[targetCityKey] || 0

    delete caster.streaks[casterCityKey]
    delete target.streaks[targetCityKey]

    caster.streaks[targetCityKey] = oppStreak
    target.streaks[casterCityKey] = myStreak
    ...
    addSkillUsageLog(
      ...
      caster.name,
      '人质交换',
      `${caster.name}对${target.name}使用了人质交换，交换了${myCityObj.name}和${oppCityObj.name}`,
      `${caster.name}使用了人质交换`
    )

    return {
      success: true,
      message: `成功交换了${myCityObj.name}和${oppCityObj.name}`
    }
  }
  ...
  function executeJieFuJiPin(caster, target) {
    ...
    // 将对手被选中的城市标记为已知（生于紫室城市除外）
    const isTargetPurple = gameStore.purpleChamber && gameStore.purpleChamber[target.name] === targetCity.city.name
    if (!isTargetPurple) {
      gameStore.setCityKnown(target.name, targetCity.city.name, caster.name)
    }
    // 将己方被选中的城市也标记为对手已知（生于紫室城市除外）
    const isMyCityPurple = gameStore.purpleChamber && gameStore.purpleChamber[caster.name] === myCity.city.name
    if (!isMyCityPurple) {
      gameStore.setCityKnown(caster.name, myCity.city.name, target.name)
    }
  ...
  function executeTianZaiRenHuo(caster, target, targetCity) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, targetCityName)) {
      addSkillUsageLog(gameStore, caster.name, '天灾人祸',
        `${caster.name}使用了天灾人祸，但目标是副中心城市，技能无效`,
        `${caster.name}使用了天灾人祸`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeBiErBuJian(caster, target, targetCity) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, targetCityName)) {
      addSkillUsageLog(gameStore, caster.name, '避而不见',
        `${caster.name}使用了避而不见，但目标是副中心城市，技能无效`,
        `${caster.name}使用了避而不见`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeShuWeiFanZhuan(caster, targetPlayer, targetCity) {
    ...
    // 确定金币花费：对对手4金币，对己方10金币
    const isOpponent = targetPlayer.name !== caster.name
    const skillCost = isOpponent ? 4 : 10
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (isOpponent && gameStore.isSubCenter(targetPlayer.name, targetCityName)) {
      addSkillUsageLog(gameStore, caster.name, '数位反转',
        `${caster.name}使用了数位反转，但目标是副中心城市，技能无效`,
        `${caster.name}使用了数位反转`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
    ...
    // 双日志
    const costMsg = isOpponent ? '花费4金币' : '花费10金币'
    addSkillUsageLog(
      gameStore,
      caster.name,
      '数位反转',
  ...
  function executeDianCiGanYing(caster, target) {
    ...
    // 先检查对手存活城市数量是否>=3
    const aliveCityCount = Object.values(target.cities).filter(
      city => city && city.isAlive !== false && (city.currentHp || city.hp) > 0
    ).length
    if (aliveCityCount < 3) {
      return { success: false, message: '对手城市数量不足3座，无法使用电磁感应' }
    }
    ...
    Object.entries(target.cities).forEach(([cityName, city]) => {
      ...
      if (gameStore.ironCities && gameStore.ironCities[target.name] &&
          gameStore.ironCities[target.name][cityName]) {
        ironEligible.push(cityName)
      } else {
        normalEligible.push(cityName)
      }
    })

    const totalEligible = ironEligible.length + normalEligible.length
    if (totalEligible < 3) {
      return { success: false, message: `对手可连接的城市不足3个（当前${totalEligible}个）` }
    }

    // 优先从钢铁城市中选择，不足则用普通城市补充
    let selectedCities = []
    if (ironEligible.length > 0) {
      const shuffledIron = ironEligible.sort(() => Math.random() - 0.5)
      selectedCities.push(...shuffledIron.slice(0, Math.min(3, shuffledIron.length)))
    }
    if (selectedCities.length < 3 && normalEligible.length > 0) {
      const shuffledNormal = normalEligible.sort(() => Math.random() - 0.5)
      selectedCities.push(...shuffledNormal.slice(0, 3 - selectedCities.length))
    }
    ...
    const now = Date.now()
    // 施法者私密日志：仅显示"A对B使用了电磁感应"
    gameStore.logs.push({
      message: `${caster.name}对${target.name}使用了电磁感应`,
      timestamp: now,
      round: gameStore.currentRound,
      isPrivate: true,
      visibleTo: [caster.name]
    })
  ...
  function executeZhongYongZhiDao(caster) {
    ...
    Object.values(caster.cities).forEach((city, idx) => {
      if (city && city.isAlive !== false && (city.currentHp || city.hp) > 0 && (city.currentHp || city.hp) < 10000) {
    ...
    gameStore.players.forEach(player => {
      ...
      Object.entries(player.cities).forEach(([cityName, city]) => {
        if (city && city.isAlive !== false && (city.currentHp || city.hp) > 0 && (city.currentHp || city.hp) > 10000) {
          // 跳过对手的副中心城市（免疫效果）
          if (gameStore.isSubCenter(player.name, cityName)) return
  ...
  function executeDangJiLiDuan(caster, target) {
    ...
    // 10. 清除对方的城市保护
    if (gameStore.protections && gameStore.protections[target.name]) {
      const count = Object.keys(gameStore.protections[target.name]).length
      if (count > 0) {
        delete gameStore.protections[target.name]
        clearedEffects.push(`城市保护×${count}`)
      }
    }

    // 11. 清除对方的高级治疗（清除healing状态和对应出战禁令）
    if (target.cities) {
      const healingCities = []
      Object.entries(target.cities).forEach(([cityName, city]) => {
        if (city && city.isInHealing) {
          city.isInHealing = false
          healingCities.push(cityName)
          // 清除对应的出战禁令
          if (gameStore.bannedCities[target.name] && gameStore.bannedCities[target.name][cityName]) {
            delete gameStore.bannedCities[target.name][cityName]
          }
        }
      })
      if (healingCities.length > 0) {
        clearedEffects.push(`高级治疗(${healingCities.length}个)`)
      }
    }

    // 12. 清除对方的城市试炼（恢复原始HP并清除禁令）
    if (gameStore.cityTrialField && gameStore.cityTrialField[target.name]) {
      const trialCities = Object.keys(gameStore.cityTrialField[target.name])
      if (trialCities.length > 0) {
        for (const cityName of trialCities) {
          const cfg = gameStore.cityTrialField[target.name][cityName]
          // 恢复原始HP
          if (cfg && cfg.originalHp && target.cities[cityName]) {
            target.cities[cityName].currentHp = cfg.originalHp
          }
          // 清除对应的出战禁令
          if (gameStore.bannedCities[target.name] && gameStore.bannedCities[target.name][cityName]) {
            delete gameStore.bannedCities[target.name][cityName]
        ...
        delete gameStore.cityTrialField[target.name]
        clearedEffects.push(`城市试炼(${trialCities.length}个，已恢复HP)`)
      }
    }
    ...
    if (target.cities) {
      let tgyjjCount = 0
      Object.values(target.cities).forEach(city => {
        if (city && city.modifiers && Array.isArray(city.modifiers)) {
          const before = city.modifiers.length
          city.modifiers = city.modifiers.filter(m => m.type !== 'mutual_destruction')
          tgyjjCount += before - city.modifiers.length
        }
      })
      if (tgyjjCount > 0) {
        clearedEffects.push(`同归于尽×${tgyjjCount}`)
      }
    }

    // 14. 清除对方的狐假虎威伪装
    if (gameStore.disguisedCities && gameStore.disguisedCities[target.name]) {
      const count = Object.keys(gameStore.disguisedCities[target.name]).length
      if (count > 0) {
        delete gameStore.disguisedCities[target.name]
        clearedEffects.push(`狐假虎威×${count}`)
      }
    }
  ...
  function executeYanTingJiCong(caster, target, targetCityName) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, targetCityName)) {
      addSkillUsageLog(gameStore, caster.name, '言听计从',
        `${caster.name}使用了言听计从，但目标是副中心城市，技能无效`,
        `${caster.name}使用了言听计从`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeShiBanGongBei(caster, skillName) {
    ...
    const SKILL_COST_MAP = {
      ...
      '擒贼擒王': 1, '草木皆兵': 3, '越战越勇': 3,
      '吸引攻击': 4, '既来则安': 4,
      '铜墙铁壁': 5,
      '背水一战': 6, '料事如神': 2, '暗度陈仓': 6,
      ...
      '借尸还魂': 4, '高级治疗': 6, '进制扭曲': 4, '苟延残喘': 4,
      '众志成城': 5, '清除加成': 5, '钢铁城市': 5, '时来运转': 5, '实力增强': 5, '城市试炼': 5, '人质交换': 4, '釜底抽薪': 5, '避而不见': 5, '劫富济贫': 5, '一触即发': 5, '技能保护': 5, '无中生有': 5, '代行省权': 5,
      '李代桃僵': 6, '天灾人祸': 6, '博学多才': 6, '城市预言': 6, '守望相助': 6, '血量存储': 6, '海市蜃楼': 8,
      '一落千丈': 7, '点石成金': 7, '寸步难行': 7, '连续打击': 7, '数位反转': 4, '倒反天罡': 7, '解除封锁': 7, '横扫一空': 17, '移花接木': 7, '连锁反应': 7, '不露踪迹': 7, '狐假虎威': 7,
      '波涛汹涌': 8, '狂轰滥炸': 8, '万箭齐发': 8, '招贤纳士': 8, '降维打击': 8, '深藏不露': 8, '定时爆破': 8, '士气大振': 11,
      '过河拆桥': 9, '厚积薄发': 9, '灰飞烟灭': 9, '电磁感应': 9, '战略转移': 9, '自相残杀': 9,
      '趁其不备': 10, '当机立断': 10,
      '搬运救兵': 11, '无懈可击': 11, '言听计从': 7,
      '副中心制': 14,
      '中庸之道': 12, '步步高升': 12, '围魏救赵': 12,
      '强制转移·普通': 13, '强制搬运': 13, '大义灭亲': 13,
      '行政中心': 15
    }
  ...
  function executeDaoFanTianGang(caster, target, capitalCityName) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, capitalCityName)) {
      addSkillUsageLog(gameStore, caster.name, '倒反天罡',
        `${caster.name}使用了倒反天罡，但目标是副中心城市，技能无效`,
        `${caster.name}使用了倒反天罡`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeBanyunJiubingPutong(caster, selfCity) {
    ...
    // 金币检查和扣除
    const goldCheck = checkAndDeductGold('搬运救兵', caster, gameStore)
    if (!goldCheck.success) {
      return goldCheck
    }
    ...
    addSkillUsageLog(
      ...
      '搬运救兵',
      `${caster.name}使用搬运救兵`,
      `你使用搬运救兵，从${provinceName}招募了${addedCities.map(c => `${c.name}(${c.hp})`).join('、')}`
    )

    return {
      success: true,
      message: `招募了2个${provinceName}城市：${addedCities.map(c => c.name).join('、')}`,
      data: {
  ...
  function executeChenqibubeiSuiji(caster, target) {
    ...
    // 检查坚不可摧护盾
    if (gameStore.isBlockedByJianbukecui(target.name, caster.name, '趁其不备')) {
      // 双日志
      addSkillUsageLog(
        gameStore,
        caster.name,
        '趁其不备',
        `${caster.name}使用趁其不备，但被${target.name}的坚不可摧护盾阻挡`,
        `${caster.name}使用了趁其不备`
      )
      return {
        success: false,
    ...
    // 金币检查和扣除
    const goldCheck = checkAndDeductGold('趁其不备', caster, gameStore)
    if (!goldCheck.success) {
      return goldCheck
    }
    ...
    // 筛选可偷城市（排除中心城市、保护城市、钢铁城市、定海神针城市、步步高升城市、谨慎交换集合城市）
    const eligible = Object.entries(target.cities)
      ...
      .filter(item => {
        ...
        if (gameStore.isInCautiousSet(target.name, item.name)) return false // 排除谨慎交换集合
        return true
      })
    ...
    addSkillUsageLog(
      ...
      caster.name,
      '趁其不备',
      `${caster.name}对${target.name}使用趁其不备，抢夺了${stolenCityName}(HP:${Math.floor(stolenCityHp)})`,
      `${caster.name}使用了趁其不备`
    )
  ...
  function executeBaQiYiZhi(caster, target, params) {
    ...
    const getEffectiveProvince = (player, cityKey) => {
      ...
      return city ? (city.province || PROVINCE_MAP[city.name]?.name) : null
    }
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, oppCityName)) {
      addSkillUsageLog(gameStore, caster.name, '拔旗易帜',
        `${caster.name}使用了拔旗易帜，但对手目标是副中心城市，技能无效`,
        `${caster.name}使用了拔旗易帜`)
      return { success: true, message: '对手目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeShouWangXiangZhu(caster, selfCity) {
    ...
    // 获取有效省份（考虑拔旗易帜changeFlagMark）
    let prov = selfCity.province || PROVINCE_MAP[selfCity.name]?.name
    if (gameStore.changeFlagMark &&
        gameStore.changeFlagMark[caster.name] &&
    ...
    // 初始化mutualWatch玩家条目（mutualWatch已在gameStore中声明为reactive）
  ...
  /**
   * 以礼来降 - 立即抢夺对手一座已知非中心、非钢铁城市，恢复初始HP，并从未使用城市池抽取同省城市（每局限2次）
   */
  function executeYiLiLaiJiang(caster, target, targetCity) {
    ...
    // 检查是否是钢铁城市
    if (gameStore.ironCities[target.name] && gameStore.ironCities[target.name][targetCityName] > 0) {
      return { success: false, message: '无法对钢铁城市使用以礼来降' }
    }

    // 检查城市是否已知（使用isCityKnown确保正确的p_前缀键访问和Set/Array兼容）
    if (!gameStore.isCityKnown(target.name, targetCityName, caster.name)) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还（含阵亡城市）
    if (gameStore.isSubCenter(target.name, targetCityName)) {
      addSkillUsageLog(gameStore, caster.name, '以礼来降',
        `${caster.name}使用了以礼来降，但目标是副中心城市，技能无效`,
        `${caster.name}使用了以礼来降`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
    ...
    // 移除对手该城市的定海神针状态
    if (gameStore.anchored?.[target.name]?.[targetCityName]) {
      delete gameStore.anchored[target.name][targetCityName]
    }

    // 移除对手该城市的生于紫室状态（被归顺后不保留）
    if (gameStore.purpleChamber && gameStore.purpleChamber[target.name] === targetCityName) {
      delete gameStore.purpleChamber[target.name]
    }

    // 移除对手该城市的副中心状态（被归顺后不保留）
    if (gameStore.subCenters && gameStore.subCenters[target.name] === targetCityName) {
      delete gameStore.subCenters[target.name]
    }
    ...
    // 被抢夺的城市对target为已知（城市现在属于caster）
    gameStore.setCityKnown(caster.name, targetCityName, target.name)
    ...
    if (bonusCity) {
      ...
      // 额外获得的城市对target为已知
      gameStore.setCityKnown(caster.name, newCity.name, target.name)
  ...
  /**
   * 大义灭亲 - 选择己方一个非直辖市城市，同省所有对手城市HP归零
   */
  function executeDaYiMieQin(caster, selfCity) {
    ...
    if (gameStore.changeFlagMark &&
      ...
      myProv = selfCity.province || PROVINCE_MAP[selfCity.name]?.name
    }
    ...
    gameStore.players.forEach(opp => {
      ...
      Object.entries(opp.cities).forEach(([cityKey, oppCity]) => {
        ...
        if (gameStore.changeFlagMark &&
          ...
          oppProv = oppCity.province || PROVINCE_MAP[oppCity.name]?.name
        }
        ...
        if (oppProv === myProv) {
          ...
          // 跳过副中心城市（免疫效果）
          if (gameStore.isSubCenter(opp.name, cityKey)) return
          ...
          if (!hasProtection) {
            oppCity.currentHp = 0
            oppCity.isAlive = false
            if (!gameStore.deadCities[opp.name]) {
              gameStore.deadCities[opp.name] = []
            }
            if (!gameStore.deadCities[opp.name].includes(cityKey)) {
              gameStore.deadCities[opp.name].push(cityKey)
            }
            destroyLog.push(`${opp.name}的${oppCity.name}`)
            totalDestroyed++
          }
        }
      })
    ...
    addSkillUsageLog(
      ...
      caster.name,
      '了大义灭亲（${myProv}）',
      `${caster.name}使用了大义灭亲（${myProv}），阵亡城市：${destroyLog.join('、')}`,
    ...
    addSkillUsageLog(
      ...
      caster.name,
      '了大义灭亲（${myProv}）',
      `${caster.name}使用了大义灭亲（${myProv}），但没有城市受影响`,
      `${caster.name}使用了了大义灭亲（${myProv}）`
    )
    }
    ...
    return {
      success: true,
      message: `${totalDestroyed}个同省对手城市阵亡`,
      data: {
        province: myProv,
  ...
  function executeQiangZhiQianDuPutong(caster, target) {
    ...
    // 检查对手是否只剩1座城市（淘汰中心后无法选择新中心）
    const aliveCount = Object.values(target.cities).filter(c => c.isAlive !== false && c.currentHp > 0).length
    if (aliveCount <= 1) {
      return {
        success: false,
        message: `${target.name}只剩1座城市，无法使用强制转移`
      }
    }
  ...
  function executeQiangZhiQianDuGaoji(caster, target, newCenterCity) {
    ...
    // 检查城市是否已知（使用isCityKnown确保正确的p_前缀键访问）
    if (!gameStore.isCityKnown(target.name, newCenterName, caster.name)) {
  ...
  // 夷为平地 - 已删除，保留空函数避免调用报错
  function executeYiWeiPingDi() {
    return { success: false, message: '该技能已被删除' }
  }
  /**
  ...
  function executeDaiXingShengQuan(caster, selfCity) {
    ...
    // 获取城市省份
    const prov = selfCity.province || PROVINCE_MAP[selfCity.name]?.name
    ...
    // 初始化actingCapital玩家条目（actingCapital已在gameStore中声明为reactive）
  ...
  function executeFuZhongXinZhi(caster, selfCity) {
    ...
    addSkillUsageLog(
      ...
      caster.name,
      '副中心制',
      `${caster.name}对${selfCity.name}使用了副中心制，该城市成为副中心，攻击力×1.5，绝对保护`,
      `${caster.name}使用了副中心制`
    )

    return {
      success: true,
      message: `${selfCity.name}成为副中心，攻击力×1.5，绝对保护`,
      data: {
        cityName: selfCity.name
      }
    }
  ...
  function executeBuBuGaoSheng(caster, selfCity) {
    ...
    if (gameStore.changeFlagMark &&
      ...
      prov = selfCity.province || PROVINCE_MAP[selfCity.name]?.name
    }
  ...
  function executeShengYuZiShi(caster, selfCity) {
    ...
    if (hasOldChamber) {
      ...
      if (centerHp > oldCityHp && oldCityHp > newCityHp) {
        ...
        // 清除旧城市的未知状态（使用setCityKnown确保正确的p_前缀键访问）
        const allPlayers = gameStore.players || []
        allPlayers.forEach(p => {
          if (p.name !== caster.name) {
            gameStore.setCityKnown(caster.name, oldChamberCityName, p.name)
          }
        })
  ...
  /**
   * 强制搬运 - 对手已知城市使用搬运救兵，并强制下回合出战这三座城市
   */
  function executeQiangZhiBanYun(caster, target, targetCity) {
    ...
    if (gameStore.changeFlagMark &&
      ...
      provinceName = targetCity.province || PROVINCE_MAP[targetCity.name]?.name
    }
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, targetCityName)) {
      addSkillUsageLog(gameStore, caster.name, '强制搬运',
        `${caster.name}使用了强制搬运，但目标是副中心城市，技能无效`,
        `${caster.name}使用了强制搬运`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeZiXiangCanSha(caster, target) {
    ...
    // 获取非中心非钢铁的存活城市（副中心可被选中，但受到的伤害为0）
    ...
    // 检查副中心免疫
    const firstIsSubCenter = gameStore.isSubCenter(target.name, firstCityKey)
    const secondIsSubCenter = gameStore.isSubCenter(target.name, secondCityKey)

    // 互相攻击：副中心城市受到的伤害为0，另一座城市正常受到效果影响
    if (firstIsSubCenter && secondIsSubCenter) {
      // 两座都是副中心（理论上不会发生，每人只有1个副中心），无伤害
      gameStore.logs.push({
        message: `${caster.name}对${target.name}使用了自相残杀，${firstCity.name}与${secondCity.name}均为副中心城市，伤害无效`,
        timestamp: Date.now(),
        round: gameStore.currentRound,
        isPrivate: true,
        visibleTo: [target.name]
      })
    } else if (firstIsSubCenter) {
      // 第一座是副中心，免疫伤害；第二座正常被第一座攻击
      ...
      gameStore.logs.push({
        message: `${caster.name}对${target.name}使用了自相残杀，${firstCity.name}（副中心，免疫伤害）与${secondCity.name}（${Math.floor(secondHp)}）互相攻击，${secondCity.name}阵亡`,
        timestamp: Date.now(),
        round: gameStore.currentRound,
        isPrivate: true,
        visibleTo: [target.name]
      })
    } else if (secondIsSubCenter) {
      // 第二座是副中心，免疫伤害；第一座正常被第二座攻击
      ...
      gameStore.logs.push({
        message: `${caster.name}对${target.name}使用了自相残杀，${firstCity.name}（${Math.floor(firstHp)}）与${secondCity.name}（副中心，免疫伤害）互相攻击，${firstCity.name}阵亡`,
        timestamp: Date.now(),
        round: gameStore.currentRound,
        isPrivate: true,
        visibleTo: [target.name]
  ...
  function executeCityDetective(caster, target, targetCityName) {
    ...
    // 副中心制保护：对手所有针对副中心城市的技能无效，金币不返还
    if (gameStore.isSubCenter(target.name, targetCityName)) {
      addSkillUsageLog(gameStore, caster.name, '城市侦探',
        `${caster.name}使用了城市侦探，但目标是副中心城市，技能无效`,
        `${caster.name}使用了城市侦探`)
      return { success: true, message: '目标是副中心城市，技能无效，金币不返还' }
    }
  ...
  function executeCityProphecy(caster, target) {
    ...
    Object.entries(target.cities).forEach(([cityName, city]) => {
      ...
      // 跳过副中心城市（城市预言不可探测）
      if (gameStore.isSubCenter(target.name, cityName)) return

      // 跳过深藏不露城市（城市预言不可探测）
      if (gameStore.hiddenGrowth && gameStore.hiddenGrowth[target.name] &&
          gameStore.hiddenGrowth[target.name][cityName]) return
  ...
  /**
   * 四面楚歌 - 归顺同省非特殊城市，未被归顺城市HP减半
   * 对手所有与己方存活城市同省的非中心非特殊城市归顺
   * 所有未被归顺的城市（包括中心和钢铁城市）HP减半
   *
  ...
  function executeSiMianChuGe(caster, target) {
    ...
    const getCityProvince = (playerName, cityKey, cityName) => {
      ...
      // 使用PROVINCE_MAP作为后备
      return PROVINCE_MAP[cityName]?.name || null
    }

    // 获取己方所有存活城市的省份集合
    const myProvinces = new Set()
    Object.entries(caster.cities).forEach(([ci, city]) => {
      const casterHp = city ? (city.currentHp !== undefined ? city.currentHp : city.hp) : 0
      if (city && casterHp > 0 && city.isAlive !== false) {
    ...
    // 第一阶段：检查与己方同省的城市，非特殊城市归顺
    const citiesToRemove = []
    for (const [ci, city] of Object.entries(target.cities)) {
      if (!city) continue
      const hp1 = city.currentHp !== undefined ? city.currentHp : city.hp
      if (hp1 <= 0 || city.isAlive === false) continue
    ...
    for (const ci of citiesToRemove) {
      ...
      // 从对手出阵列表中移除
      if (gameStore.roster && gameStore.roster[target.name]) {
        gameStore.roster[target.name] = gameStore.roster[target.name]
          .filter(rName => rName !== ci)
      }
    }

    // centerCityName 不需要调整（已经是城市名称）

    // 第二阶段：对手所有未被归顺的城市（包括中心和钢铁城市）HP减半
    for (const [ci, city] of Object.entries(target.cities)) {
      if (!city) continue
      const hp2 = city.currentHp !== undefined ? city.currentHp : city.hp
      if (hp2 <= 0 || city.isAlive === false) continue
    ...
    for (let idx = 0; idx < transferredCities.length; idx++) {
      ...
      if (!gameStore.roster[caster.name].includes(newKey)) {
        gameStore.roster[caster.name].push(newKey)
      }
    ...
    if (halvedCities.length > 0) {
      logMsg += `${transferredCities.length > 0 ? '；' : '，'}${halvedCities.length}座城市HP减半`
    }
    ...
    return {
      success: true,
      message: `四面楚歌！${transferredCities.length > 0 ? `归顺${transferredCities.length}座城市` : ''}${halvedCities.length > 0 ? ` 减半${halvedCities.length}座城市` : ''}`,
      data: {
  ...
  /**
   * 博学多才 - 答题增加HP
   * 选择己方1座城市，回答3道该城市相关知识的单选题
   * 根据答对数量增加HP：0题x1, 1题x1.25, 2题x1.75, 3题x2.5
   *
   * @param {Object} caster - 施法者
   ...
   * @returns {Object} { success, message, data }
   */
  function executeBoXueDuoCai(caster, cityName, correctCount = 0, options = {}) {
    ...
    // 金币检查和扣除（skipGold用于拦截流程后的答题阶段，金币已在准备阶段扣除）
    if (!options.skipGold) {
    ...
    if (correctCount < 0) {
      ...
      return { success: true, message: '博学多才已发动，等待答题', needsQuiz: true }
    }
    ...
    // 计算HP倍数：0题x1, 1题x1.25, 2题x1.75, 3题x2.5
    const hpMultipliers = [1, 1.25, 1.75, 2.5]
