...
export function prepareBattleAnimationData(roomData, gameStore) {
  debugLog('[BattleAnimationData] ===== 开始准备战斗动画数据 =====')
  debugLog('[BattleAnimationData] roomData存在:', !!roomData)
  debugLog('[BattleAnimationData] roomData.mode:', roomData?.mode)
  debugLog('[BattleAnimationData] roomData.players数量:', roomData?.players?.length)
  debugLog('[BattleAnimationData] roomData.gameState存在:', !!roomData?.gameState)
  ...
  if (!players || players.length < 2) {
    // fallback: 如果roomData.players数据不完整，尝试使用gameStore.players
    if (gameStore && gameStore.players && gameStore.players.length >= 2) {
      console.warn('[BattleAnimationData] roomData.players不足，使用gameStore.players作为备用')
  ...
  debugLog('[BattleAnimationData] player1:', player1?.name, '城市数量:', Object.keys(player1?.cities || {}).length)
  debugLog('[BattleAnimationData] player2:', player2?.name, '城市数量:', Object.keys(player2?.cities || {}).length)
  debugLog('[BattleAnimationData] gameState.playerStates存在:', !!gameState?.playerStates)
  debugLog('[BattleAnimationData] gameState.playerStates keys:', gameState?.playerStates ? Object.keys(gameState.playerStates) : 'undefined')

  const state1 = gameState.playerStates[player1.name]
  const state2 = gameState.playerStates[player2.name]

  debugLog('[BattleAnimationData] state1存在:', !!state1)
  debugLog('[BattleAnimationData] state2存在:', !!state2)
  ...
  if (specialEvent && gameState.specialEventThisRound) {
    debugLog('[BattleAnimationData] 从specialEventThisRound恢复出战城市信息')
    ...
    if (hasSurrenderObjects || hasRetreatObjects) {
      debugLog('[BattleAnimationData] 使用specialEventThisRound中保存的完整城市对象')
      ...
      debugLog('[BattleAnimationData] 已使用保存的城市对象构建动画数据')
      debugLog('[BattleAnimationData] player1出战城市:', battleCities1.map(c => c.name))
      debugLog('[BattleAnimationData] player2出战城市:', battleCities2.map(c => c.name))
    } else {
      // 降级方案：使用城市名称查找（旧逻辑）
      debugLog('[BattleAnimationData] specialEventThisRound中没有完整城市对象，尝试通过名称查找')
      ...
      debugLog('[BattleAnimationData] player1城市名称:', player1CityNames)
      debugLog('[BattleAnimationData] player2城市名称:', player2CityNames)
  ...
  debugLog('[BattleAnimationData] 特殊事件检测结果:', specialEvent)
  debugLog('[BattleAnimationData] player1出战城市:', battleCities1.map(c => c.name))
  debugLog('[BattleAnimationData] player2出战城市:', battleCities2.map(c => c.name))
  ...
  debugLog('[BattleAnimationData] player1总攻击力:', totalAttack1)
  debugLog('[BattleAnimationData] player2总攻击力:', totalAttack2)
  ...
  debugLog('[BattleAnimationData] ===== 动画数据准备完成 =====')
  debugLog('[BattleAnimationData] animData.round:', animData.round)
  debugLog('[BattleAnimationData] animData.player1.name:', animData.player1.name)
  debugLog('[BattleAnimationData] animData.player1.cities数量:', animData.player1.cities.length)
  debugLog('[BattleAnimationData] animData.player2.name:', animData.player2.name)
  debugLog('[BattleAnimationData] animData.player2.cities数量:', animData.player2.cities.length)
  debugLog('[BattleAnimationData] animData.specialEvent:', animData.specialEvent)
  debugLog('[BattleAnimationData] animData.fatigueData数量:', animData.fatigueData.length)
  debugLog('[BattleAnimationData] ========================================')
...
function checkSameProvinceRetreat(player1, player2, state1, state2) {
  ...
  debugLog('[BattleAnimationData] 检查同省撤退:')
  debugLog('[BattleAnimationData] cities1:', cities1.map(c => ({ name: c.name, province: c.province, isCapital: c.isCapital, isProvincialCapital: c.isProvincialCapital })))
  debugLog('[BattleAnimationData] cities2:', cities2.map(c => ({ name: c.name, province: c.province, isCapital: c.isCapital, isProvincialCapital: c.isProvincialCapital })))

  // 检查是否有同省城市
  for (const city1 of cities1) {
    for (const city2 of cities2) {
      if (city1.province === city2.province && city1.province) {
        debugLog(`[BattleAnimationData] 发现同省城市: ${city1.name} (${city1.province}) vs ${city2.name} (${city2.province})`)

        // 检查是否都不是省会
        const isCapital1 = city1.isCapital || city1.isProvincialCapital
        const isCapital2 = city2.isCapital || city2.isProvincialCapital

        debugLog(`[BattleAnimationData] ${city1.name} 是否省会: ${isCapital1}, ${city2.name} 是否省会: ${isCapital2}`)

        if (!isCapital1 && !isCapital2) {
          debugLog(`[BattleAnimationData] ✓ 确认同省撤退: ${city1.province}`)
          return {
            province: city1.province
          }
        } else {
          debugLog(`[BattleAnimationData] ✗ 不符合同省撤退条件（有省会）`)
        }
      }
    }
  }

  debugLog('[BattleAnimationData] 未检测到同省撤退')
  return null
}
...
export function updateBattleResults(battleAnimationData, roomData) {
  debugLog('[BattleAnimationData] 更新战斗结果')
...
function collectFatigueData(battleAnimationData, roomData, battleResults) {
  debugLog('[BattleAnimationData] 收集疲劳减半数据')
  ...
  if (!battleAnimationData.fatigueData || battleAnimationData.fatigueData.length === 0) {
    debugLog('[BattleAnimationData] 本回合无疲劳城市')
    return
  }
  ...
  battleAnimationData.fatigueData.forEach(fatigue => {
    ...
    if (animCityIdx !== -1) {
      ...
      debugLog(`[BattleAnimationData] 添加疲劳: player${playerNum} ${fatigue.cityName} (animIdx=${animCityIdx}, HP: ${Math.floor(fatigue.hpBefore)}→${Math.floor(fatigue.hpAfter)})`)
  ...
  debugLog('[BattleAnimationData] 疲劳数据收集完成，数量:', battleResults.fatigued.length)
}
