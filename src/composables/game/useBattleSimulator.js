...
export function calculateCityPower(city, cityName, player, gameStore) {
  debugLog(`[calculateCityPower] 城市:${city?.name}, cityName:${cityName}, isAlive:${city?.isAlive}, currentHp:${city?.currentHp}, hp:${city?.hp}`)

  // 修复：显式检查 isAlive === false，而不是 !isAlive（避免undefined被当作false）
  if (city.isAlive === false) {
    debugLog(`[calculateCityPower] ${city.name} 已阵亡（isAlive=false），返回攻击力0`)
    return 0
  }
  ...
  if (!power || power <= 0) {
    debugLog(`[calculateCityPower] ${city.name} HP为0或undefined，返回攻击力0`)
    return 0
  }
  ...
  debugLog(`[calculateCityPower] ${city.name} 最终攻击力:${Math.floor(power)}`)
...
export function simulateBattle(attackerCity, attackerCityName, defenderCity, defenderCityName, attackerPlayer, defenderPlayer, gameStore) {
  ...
  // === 既来则安保护（本轮免疫伤害） ===
  const jlzaState = gameStore.jilaizan?.[defenderPlayer.name]?.[defenderCity.name]
  if (jlzaState) {
    return {
      success: true,
      attackPower,
...
export function calculateBattleResult(
  ...
  debugLog('[calculateBattleResult] ===== 战斗结果计算 =====')
  debugLog('[calculateBattleResult] 攻击方:', attackerPlayer.name)
  debugLog('[calculateBattleResult] attackerCities 长度:', attackerCities.length)
  attackerCities.forEach((city, i) => {
    debugLog(`  [${i}] name=${city.name}, cityName=${city.cityName}, hp=${city.hp}, currentHp=${city.currentHp}, isAlive=${city.isAlive}`)
  })

  // 检查是否有同省撤退或省会归顺事件
  // 如果有，攻击力为0，不造成伤害
  debugLog('[calculateBattleResult] 检查specialEventThisRound:', gameStore.specialEventThisRound)
  if (gameStore.specialEventThisRound) {
    const event = gameStore.specialEventThisRound
    debugLog('[calculateBattleResult] 特殊事件类型:', event.type)
    if (event.type === 'retreat' || event.type === 'surrender') {
      debugLog(`[calculateBattleResult] ✅ 检测到特殊事件: ${event.type}，攻击力设为0`)
      return {
        totalAttackPower: 0,
        netDamage: 0,
        destroyedCities: [],
  ...
  let totalAttackPower = attackerCities.reduce((sum, city) => {
    ...
    debugLog(`  [calculateCityPower] ${city.name} (name=${cityName}): power=${power}`)
    return sum + power
  }, 0)

  // 草木皆兵等技能的伤害倍率
  if (battleSkills.damageMultiplier && battleSkills.damageMultiplier !== 1) {
    debugLog(`[calculateBattleResult] 应用伤害倍率: ${battleSkills.damageMultiplier}`)
    totalAttackPower = Math.floor(totalAttackPower * battleSkills.damageMultiplier)
  }

  debugLog('[calculateBattleResult] 总攻击力:', totalAttackPower)
  ...
  if (gameStore.barrier?.[defenderPlayer.name] && gameStore.barrier[defenderPlayer.name].hp > 0) {
    ...
    debugLog(`[calculateBattleResult] 屏障吸收: ${barrierAbsorbed}, 反弹: ${barrierReflected}, 剩余屏障HP: ${barrier.hp}`)
    if (barrier.hp <= 0) {
      delete gameStore.barrier[defenderPlayer.name]
      debugLog(`[calculateBattleResult] 屏障被摧毁`)
    }
  }
  ...
  for (const city of targetOrder) {
    ...
    // 检查既来则安保护（本轮免疫伤害）
    const jlza = gameStore.jilaizan?.[defenderPlayer.name]?.[city.name]
    if (jlza) {
      debugLog(`[calculateBattleResult] ${city.name} 受到既来则安保护，免疫伤害`)
      continue
    }
  ...
  // === 电磁感应连锁反应 ===
  // 计算连锁伤害映射，应用到已部署城市，并返回完整映射供battle2P处理未部署城市
  let electromagneticChainDamage = null
  if (gameStore.electromagnetic?.[defenderPlayer.name]) {
    ...
    for (const [cityName, chainDmg] of Object.entries(chainDamageMap)) {
      ...
      debugLog(`[calculateBattleResult] 电磁感应连锁: ${cityName} 受到 ${actualChainDmg} 连锁伤害（已部署）`)
      ...
      // 已处理的城市从chainDamageMap中移除，避免battle2P重复应用
      delete chainDamageMap[cityName]
    }

    // 返回未处理的连锁伤害（未部署城市），由battle2P直接应用到player.cities
    if (Object.keys(chainDamageMap).length > 0) {
      electromagneticChainDamage = chainDamageMap
    }
  }
