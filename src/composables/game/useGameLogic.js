...
export function useGameLogic() {
  ...
  function initializeGame(playerName, mode, drawnCities) {
    ...
    debugLog('Game initialized:', gameStore.players)
  }
  ...
  function useSkill(skillName, params) {
    ...
    debugLog('Using skill:', skillName, params)
  ...
  function endTurn() {
    ...
    debugLog('[endTurn] nextIndex:', nextIndex, 'players.length:', gameStore.players.length)
    ...
    // 如果回到第一个玩家，增加回合数
    debugLog('[endTurn] 检查是否回到第一个玩家: nextIndex === 0?', nextIndex === 0)
    if (nextIndex === 0) {
      debugLog('[endTurn] 回到第一个玩家，调用processNewRound()')
  ...
  function processNewRound() {
    ...
    debugLog('[processNewRound] goldLoanRounds状态:', JSON.stringify(gameStore.goldLoanRounds))
    ...
    // 4. 每回合给予金币（原版：+3金币/回合）
    gameStore.players.forEach(player => {
      debugLog(`[processNewRound] 处理玩家 ${player.name} 的金币，当前金币: ${player.gold}`)
      debugLog(`[processNewRound] ${player.name} 的 goldLoanRounds: ${gameStore.goldLoanRounds[player.name]}`)
  ...
  function isPlayerDefeated(player) {
    ...
    debugLog(`[isPlayerDefeated] 检查玩家 ${player.name}:`, {
  ...
  function executeAITurn(aiPlayer) {
    debugLog('AI turn:', aiPlayer.name)
