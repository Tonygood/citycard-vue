  ...
  for (let attacker = 0; attacker < 2; attacker++) {
    ...
    for (const prov in provinceMap[attacker]) {
      ...
      if ((defenderHasRealCapital || defenderHasActingCapital) && defenderSurrenderableCities.length === 0) {
        ...
        // 标记所有出战城市为已知（生于紫室城市除外）
        sel0.forEach(cn => {
          if (gameStore.purpleChamber[player0.name] === cn) return
          gameStore.setCityKnown(player0.name, cn, player1.name)
        })
        sel1.forEach(cn => {
          if (gameStore.purpleChamber[player1.name] === cn) return
          gameStore.setCityKnown(player1.name, cn, player0.name)
        })
      ...
      if (hasCapital && defenderSurrenderableCities.length > 0) {
        ...
        if (citiesToTransfer.length > 0) {
          debugLog(`[PreBattleChecks] 开始转移${citiesToTransfer.length}个城市从${defenderPlayer.name}到${attackerPlayer.name}`)

          // 1. 将城市添加到攻击方
          citiesToTransfer.forEach(({ cityName, city }) => {
            attackerPlayer.cities[cityName] = city
            debugLog(`[PreBattleChecks] 已将${city.name}添加到${attackerPlayer.name}的城市列表`)
          })

          // 2. 从防守方移除城市
          citiesToTransfer.forEach(({ cityName }) => {
            delete defenderPlayer.cities[cityName]
            debugLog(`[PreBattleChecks] 已从${defenderPlayer.name}移除城市${cityName}`)
          })
          ...
          if (attackerGameStorePlayer && defenderGameStorePlayer) {
            ...
            citiesToTransfer.forEach(({ cityName }) => {
              if (defenderGameStorePlayer.streaks[cityName]) {
                ...
                debugLog(`[PreBattleChecks] 转移疲劳记录: ${defenderPlayer.name}[${cityName}] -> ${attackerPlayer.name}[${cityName}], streak=${attackerGameStorePlayer.streaks[cityName]}`)
          ...
          debugLog(`[PreBattleChecks] 城市转移完成：${attackerPlayer.name}现有${Object.keys(attackerPlayer.cities).length}个城市，${defenderPlayer.name}现有${Object.keys(defenderPlayer.cities).length}个城市`)
        }

        // 关键修复Bug1: 标记所有出战城市为已知（双向，生于紫室城市除外）
        // 攻击方的所有出战城市被防守方知道
        sel0.forEach(cityName => {
          ...
          // 生于紫室城市不标记为已知
          if (gameStore.purpleChamber[player0.name] === cityName) return
        ...
        sel1.forEach(cityName => {
          ...
          // 生于紫室城市不标记为已知
          if (gameStore.purpleChamber[player1.name] === cityName) return
          // 参数顺序：setCityKnown(拥有者, 城市名称, 观察者)
          if (attacker === 1) {
            // player1的城市被player0知道
            gameStore.setCityKnown(player1.name, cityName, player0.name)
        ...
        if (attackerGameStorePlayer && defenderGameStorePlayer) {
          ...
          debugLog(`[PreBattleChecks] 归顺streak累积 - 攻击方出战城市数:`, attackerBattleCities.length)
          debugLog(`[PreBattleChecks] 归顺streak累积 - 防守方出战城市数:`, defenderBattleCities.length)
          ...
          attackerBattleCities.forEach(card => {
            ...
            debugLog(`[PreBattleChecks] 归顺：${attackerPlayer.name}的城市${cityName} streak: ${oldStreak} → ${attackerGameStorePlayer.streaks[cityName]}`)
          })
          ...
          defenderBattleCities.forEach(card => {
            ...
            debugLog(`[PreBattleChecks] 归顺：${defenderPlayer.name}的城市${cityName} streak: ${oldStreak} → ${defenderGameStorePlayer.streaks[cityName]}`)
          })
          ...
          debugLog(`[PreBattleChecks] 归顺：同步streak到players数组`)
          debugLog(`[PreBattleChecks] ${attackerPlayer.name} streaks:`, attackerPlayer.streaks)
          debugLog(`[PreBattleChecks] ${defenderPlayer.name} streaks:`, defenderPlayer.streaks)
    ...
    if (provinceMap[1][prov] && provinceMap[1][prov].length > 0) {
      ...
      if (player0GameStore && player1GameStore) {
        ...
        sel0.forEach(cityName => {
          ...
          debugLog(`[PreBattleChecks] 同省撤退：${player0.name}的城市[${cityName}] ${city?.name} streak: ${oldStreak} → ${player0GameStore.streaks[cityName]}`)
        })
        ...
        sel1.forEach(cityName => {
          ...
          debugLog(`[PreBattleChecks] 同省撤退：${player1.name}的城市[${cityName}] ${city?.name} streak: ${oldStreak} → ${player1GameStore.streaks[cityName]}`)
        })
        ...
        debugLog(`[PreBattleChecks] 同省撤退：同步streak到players数组`)
        debugLog(`[PreBattleChecks] ${player0.name} streaks:`, player0.streaks)
        debugLog(`[PreBattleChecks] ${player1.name} streaks:`, player1.streaks)
      }
...
export function executePreBattleChecks(gameStore, gameState, players, mode) {
  debugLog('[PreBattleChecks] 开始执行战斗前检测')
  ...
  debugLog('[PreBattleChecks] 战斗前检测完成，继续正常战斗流程')
