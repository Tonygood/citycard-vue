...
export function applyFatigueReduction(gameStore, gameState, players, mode) {
  ...
  players.forEach((player, playerIdx) => {
    ...
    debugLog(`[疲劳系统] ===== ${player.name} 疲劳检查开始 =====`)
    debugLog(`[疲劳系统] ${player.name} 本轮出战城市名称:`, deployedCities)
    debugLog(`[疲劳系统] ${player.name} 当前所有streaks:`, JSON.stringify(player.streaks, null, 2))
    ...
    deployedCities.forEach(cityName => {
      ...
      debugLog(`[疲劳系统] ${player.name} 的 ${city.name} 上一轮streak=${prevStreak}, 当前HP=${city.currentHp}`)
      ...
      if (prevStreak >= 1 && !hasYYY && !hasJLZA && !hasIgnoreFatigue) {
        ...
        // 注意：不更新city.hp，city.hp保持为初始/最大HP
        ...
        debugLog(`[疲劳系统] ${player.name} 的 ${city.name} HP减半: ${beforeHp} → ${city.currentHp}`)
      } else if (prevStreak >= 1) {
        const reason = hasYYY ? '越战越勇' : hasJLZA ? '既来则安' : 'ignore_fatigue'
        debugLog(`[疲劳系统] ${player.name} 的 ${city.name} 因${reason}豁免疲劳减半`)
      }
    })
  })

  debugLog('[疲劳系统] 疲劳减半检查完成')
}
...
export function updateFatigueStreaks(players, gameState, mode) {
  ...
  if (gameState.specialEventThisRound) {
    debugLog('[疲劳系统] 本回合有特殊事件（同省撤退/归顺），streaks已在preBattleChecks中更新，跳过')
    return
  }
  ...
  players.forEach(player => {
    ...
    debugLog(`[疲劳系统] ${player.name} 本轮出战城市:`, deployedCities)
    ...
    Object.entries(player.cities).forEach(([cityName, city]) => {
      ...
      if (deployedSet.has(cityName)) {
        ...
        debugLog(`[疲劳系统] ${player.name} 的 ${city.name} streak: ${oldStreak} → ${player.streaks[cityName]}`)
      } else {
        // 未出战城市：归零
        if (player.streaks[cityName] > 0) {
          debugLog(`[疲劳系统] ${player.name} 的 ${city.name} 未出战，streak归零: ${player.streaks[cityName]} → 0`)
        }
        player.streaks[cityName] = 0
      }
    })
...
function checkJiLaiZeAn(gameStore, player, cityName) {
  ...
  return !!gameStore.jilaizan[player.name][cityName]
}
