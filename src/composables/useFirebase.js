...
export function initializeFirebase(config = null) {
  try {
    debugLog('[Firebase] 开始初始化 Firebase...', {
    ...
    // 如果已经初始化，先删除
    if (firebaseApp) {
      debugLog('[Firebase] 删除已存在的 Firebase 实例')
    ...
    debugLog('[Firebase] ✓ Firebase 初始化成功！isInitialized =', isInitialized)
    return true
  } catch (error) {
...
export function autoInitFirebase() {
  debugLog('[Firebase] 自动初始化检查...')

  // 优先使用保存的配置
  const savedConfig = loadFirebaseConfig()
  if (savedConfig) {
    debugLog('[Firebase] 检测到已保存的自定义配置，使用自定义配置初始化')
    return initializeFirebase(savedConfig)
  }

  // 如果有默认配置，自动使用
  if (hasDefaultConfig()) {
    debugLog('[Firebase] 检测到默认配置，使用默认配置初始化')
    return initializeFirebase(DEFAULT_FIREBASE_CONFIG)
  }

  debugLog('[Firebase] 未找到 Firebase 配置，将使用本地存储模式')
  return false
}
