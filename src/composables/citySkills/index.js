...
export function processActivatedCitySkills(attacker, attackerState, defender, defenderCities, addPublicLog, gameStore) {
  debugLog('[城市技能] processActivatedCitySkills 被调用')
  debugLog('[城市技能] attacker:', attacker.name)
  debugLog('[城市技能] attackerState.activatedCitySkills:', attackerState.activatedCitySkills)

  if (!attackerState.activatedCitySkills) {
    debugLog('[城市技能] 没有激活的城市技能，跳过处理')
    return
  }

  const skillCount = Object.keys(attackerState.activatedCitySkills).length
  debugLog(`[城市技能] ${attacker.name} 共激活了 ${skillCount} 个城市技能`)

  Object.keys(attackerState.activatedCitySkills).forEach(cityName => {
    const skillData = attackerState.activatedCitySkills[cityName]
    debugLog(`[城市技能] 处理技能: ${skillData.skillName} (城市名: ${cityName})`)

    const handler = SKILL_HANDLERS[skillData.skillName]

    if (handler) {
      debugLog(`[城市技能] 找到处理函数，执行: ${skillData.skillName}`)
      try {
        // 根据技能类型调用不同的处理函数签名
        ...
        debugLog(`[城市技能] 技能 ${skillData.skillName} 执行成功`)
      } catch (error) {
