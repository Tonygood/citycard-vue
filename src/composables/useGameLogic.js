...
export function useGameLogic() {
  ...
  function addPrivateLog(playerName, message) {
    ...
    debugLog(`[私有日志-${playerName}] ${message}`)
  }
  ...
  function battle2P(players, gameState) {
    ...
    // 获取出战城市
    debugLog('[战斗] ===== 开始战斗计算 =====')
    debugLog('[战斗] player1:', player1.name, 'cities数量:', Object.keys(player1.cities).length)
    debugLog('[战斗] player2:', player2.name, 'cities数量:', Object.keys(player2.cities).length)
    ...
    const cities1 = rawBattle1.filter(card => card && card.cityName).map((card, mapIdx) => {
      ...
      debugLog(`[战斗诊断] ${player1.name} [${mapIdx}] cityName=${card.cityName}, resolved=${resolvedName}, city.name=${city?.name}, city.currentHp=${city?.currentHp}, city.hp=${city?.hp}, city.isAlive=${city?.isAlive}`)
      if (!city) {
        console.error(`[战斗错误] ${player1.name} cityName=${card.cityName} 对应的城市不存在！`)
        return null
      }
      // 关键修复：过滤已阵亡城市（防止Firebase同步延迟导致部署了已死城市）
      const hp = city.currentHp !== undefined ? city.currentHp : city.hp
      if (city.isAlive === false || hp <= 0) {
        console.warn(`[战斗] ${player1.name} 的 ${city.name} 已阵亡(hp=${hp}, isAlive=${city.isAlive})，从出战列表移除（可能因同步延迟被错误部署）`)
        addPublicLog(`${player1.name} 的 ${city.name} 已阵亡，无法出战`)
        return null
      }
      return {
        ...city,
    ...
    const cities2 = rawBattle2.filter(card => card && card.cityName).map((card, mapIdx) => {
      ...
      debugLog(`[战斗诊断] ${player2.name} [${mapIdx}] cityName=${card.cityName}, resolved=${resolvedName}, city.name=${city?.name}, city.currentHp=${city?.currentHp}, city.hp=${city?.hp}, city.isAlive=${city?.isAlive}`)
      if (!city) {
      ...
      const hp = city.currentHp !== undefined ? city.currentHp : city.hp
      if (city.isAlive === false || hp <= 0) {
        console.warn(`[战斗] ${player2.name} 的 ${city.name} 已阵亡(hp=${hp}, isAlive=${city.isAlive})，从出战列表移除（可能因同步延迟被错误部署）`)
        addPublicLog(`${player2.name} 的 ${city.name} 已阵亡，无法出战`)
        return null
      }
      return {
        ...city,
    ...
    // 关键修复：在城市技能处理之前，保存双方攻击方快照
    // processActivatedCitySkills会修改cities1/cities2（作为defenderCities参数），
    // 但攻击力计算应使用技能处理前的原始HP数据，确保双向战斗真正"同时"进行
    const attackerSnapshot1 = JSON.parse(JSON.stringify(cities1))
    const attackerSnapshot2 = JSON.parse(JSON.stringify(cities2))

    // 处理城市专属技能激活效果
    debugLog('[battle2P] ===== 处理城市专属技能激活 =====')
    debugLog('[battle2P] player1:', player1.name)
    debugLog('[battle2P] state1.activatedCitySkills:', state1.activatedCitySkills)
    debugLog('[battle2P] player2:', player2.name)
    debugLog('[battle2P] state2.activatedCitySkills:', state2.activatedCitySkills)
    ...
    // 如果有，双方攻击力都为0，不造成伤害
    debugLog('[battle2P] 检查specialEventThisRound:', gameState.specialEventThisRound)
    let totalAttack1 = 0
    let totalAttack2 = 0

    if (gameState.specialEventThisRound) {
      const event = gameState.specialEventThisRound
      debugLog('[battle2P] 特殊事件类型:', event.type)
      if (event.type === 'retreat' || event.type === 'surrender') {
        debugLog(`[battle2P] ✅ 检测到特殊事件: ${event.type}，双方攻击力设为0`)
        totalAttack1 = 0
        totalAttack2 = 0
    ...
    if (!gameState.specialEventThisRound ||
      ...
      attackerSnapshot2.forEach(city => {
        if (city.hp > 0) {
    ...
    if (totalAttack1 > 0 && cities2.length > 0) {
      ...
      // 关键修复：使用城市技能处理前的快照作为攻击方数据
      // processActivatedCitySkills可能修改了cities1（当Player2的技能作用于Player1的城市时），
      // 但攻击力应基于技能处理前的原始HP，确保双向战斗同时进行不互相影响
      const attackerCitiesWithName = attackerSnapshot1.map(c => ({ ...c }))

      // 获取防守方城市的完整对象（含城市名）- 深度克隆避免被修改
      // 防守方使用cities2（已被城市技能处理过的状态，如HP减半等）
      const defenderCitiesWithName = cities2.map(c => {
        const city = player2.cities[c.cityName]
        if (!city) {
          console.warn(`[battle2P] 防守方城市 ${c.cityName} 已不存在（可能被城市技能移除）`)
          return null
        }
        // 使用player.cities的当前状态（城市技能可能修改了防守方城市的属性）
    ...
    if (totalAttack2 > 0 && cities1.length > 0) {
      ...
      // 关键修复：使用城市技能处理前的快照作为攻击方数据
      // 确保双向战斗计算真正同时进行，不受中间修改（城市技能/Firebase同步）影响
      const attackerCitiesWithName = attackerSnapshot2.map(c => ({ ...c }))

      // 获取防守方城市的完整对象（含城市名）- 深度克隆避免被修改
      // 防守方使用cities1（已被城市技能处理过的状态，如HP减半等）
    ...
    if (defenderCities1) {
      ...
      if (battleResult1.destroyedCities) {
        battleResult1.destroyedCities.forEach(cityName => {
          ...
          if (gameStore.anchored?.[player2.name]?.[cityName]) {
            delete gameStore.anchored[player2.name][cityName]
          }
        })
      }
    }
    ...
    if (defenderCities2) {
      ...
      if (battleResult2.destroyedCities) {
        battleResult2.destroyedCities.forEach(cityName => {
          ...
          if (gameStore.anchored?.[player1.name]?.[cityName]) {
            delete gameStore.anchored[player1.name][cityName]
          }
        })
      }
    }
    ...
    // === 电磁感应连锁伤害：应用到未部署的城市 ===
    // calculateBattleResult已处理了部署中的连锁城市，这里处理未部署的连锁城市
    if (battleResult1 && battleResult1.electromagneticChainDamage) {
      Object.entries(battleResult1.electromagneticChainDamage).forEach(([cityName, chainDmg]) => {
        const originalCity = player2.cities[cityName]
        if (!originalCity || originalCity.isAlive === false) return
        const hp = originalCity.currentHp !== undefined ? originalCity.currentHp : originalCity.hp
        if (hp <= 0) return

        const actualDmg = Math.min(hp, chainDmg)
        originalCity.currentHp = hp - actualDmg

        addPublicLog(`⚡ 电磁感应连锁: ${player2.name}的${cityName} 受到 ${actualDmg} 连锁伤害`)
    ...
    if (battleResult2 && battleResult2.electromagneticChainDamage) {
      Object.entries(battleResult2.electromagneticChainDamage).forEach(([cityName, chainDmg]) => {
        ...
        addPublicLog(`⚡ 电磁感应连锁: ${player1.name}的${cityName} 受到 ${actualDmg} 连锁伤害`)
    ...
    if (gameStore.financialCrisis && gameStore.financialCrisis.roundsLeft > 0) {
      // 金融危机：金币最高的玩家不获得金币，其余获得1
      const maxGold = Math.max(player1.gold, player2.gold)
      ;[player1, player2].forEach(p => {
        if (p.gold === maxGold) {
          addPublicLog(`${p.name} 当前金币最高，因金融危机影响，本回合不获得自动金币`)
        } else {
          p.gold = Math.min(24, p.gold + 1)
          addPublicLog(`${p.name} 因金融危机影响，本回合获得1金币（而非3金币）`)
        }
      })
    } else {
      const base = 3
    ...
    // 清空本轮既来则安标记
    Object.keys(gameStore.jilaizan).forEach(key => delete gameStore.jilaizan[key])
  ...
  function checkRosterRefillNeeded(player, playerState) {
    ...
    if (keepInRoster.length < rosterLimit) {
      ...
      debugLog(`[checkRosterRefillNeeded] ${player.name} 自动补充roster: +${autoFill.length}个城市`)
      return
    }
  ...
  function checkWinCondition(players, gameState) {
    ...
    const alivePlayers = players.filter(player => {
      // 关键修复：如果玩家正在选择新中心城市（强制转移·普通等），不判定为已输
      // needsNewCenter表示中心城市被技能摧毁，但玩家还有机会选择新中心
      const playerState = gameStore.playerStates?.[player.name]
      if (playerState?.needsNewCenter) {
        return true // 暂不判负，等待选择新中心
      }
      ...
      otherPlayers.forEach(defender => {
        ...
        if (attackingCities.length > 0) {
          ...
          if (attackerCitiesData.length > 0) {
            ...
            // 电磁感应连锁伤害：应用到未部署的城市
            if (battleResult.electromagneticChainDamage) {
              Object.entries(battleResult.electromagneticChainDamage).forEach(([cityName, chainDmg]) => {
                const originalCity = defender.cities[cityName]
                if (!originalCity || originalCity.isAlive === false) return
                const hp = originalCity.currentHp !== undefined ? originalCity.currentHp : originalCity.hp
                if (hp <= 0) return
                ...
                const actualDmg = Math.min(hp, chainDmg)
                originalCity.currentHp = hp - actualDmg
                addPublicLog(`⚡ 电磁感应连锁: ${defender.name}的${cityName} 受到 ${actualDmg} 连锁伤害`)
                if (originalCity.currentHp <= 0) {
                  originalCity.currentHp = 0
                  originalCity.isAlive = false
                  if (!defenderState.deadCities) defenderState.deadCities = []
  ...
  function battle2v2(players, gameState) {
    ...
    if (team1Cities.length > 0 && team0Cities.length > 0 && (!barrier || !barrier.active || barrier.team !== '红队')) {
      ...
      // 电磁感应连锁伤害：应用到未部署的红队城市
      if (battleResult.electromagneticChainDamage) {
        Object.entries(battleResult.electromagneticChainDamage).forEach(([cityName, chainDmg]) => {
          const cityData = team0Cities.find(c => c.city.name === cityName) ||
            (() => { for (const p of team0) { if (p.cities[cityName]) return { player: p, cityName } } return null })()
          if (!cityData) return
          const player = cityData.player || cityData
          const originalCity = player.cities?.[cityName]
          if (!originalCity || originalCity.isAlive === false) return
          const hp = originalCity.currentHp !== undefined ? originalCity.currentHp : originalCity.hp
          if (hp <= 0) return
          ...
          const actualDmg = Math.min(hp, chainDmg)
          originalCity.currentHp = hp - actualDmg
          addPublicLog(`⚡ 电磁感应连锁: ${cityName} 受到 ${actualDmg} 连锁伤害`)
          if (originalCity.currentHp <= 0) {
            originalCity.currentHp = 0
            originalCity.isAlive = false
            const state = gameState.playerStates[player.name]
            if (state) {
              if (!state.deadCities) state.deadCities = []
              if (!state.deadCities.includes(cityName)) state.deadCities.push(cityName)
            }
          }
        })
      }
    ...
    if (team0Cities.length > 0 && team1Cities.length > 0 && (!barrier || !barrier.active || barrier.team !== '蓝队')) {
      ...
      // 电磁感应连锁伤害：应用到未部署的蓝队城市
      if (battleResult.electromagneticChainDamage) {
        Object.entries(battleResult.electromagneticChainDamage).forEach(([cityName, chainDmg]) => {
          const cityData = team1Cities.find(c => c.city.name === cityName) ||
            (() => { for (const p of team1) { if (p.cities[cityName]) return { player: p, cityName } } return null })()
          if (!cityData) return
          const player = cityData.player || cityData
          const originalCity = player.cities?.[cityName]
          if (!originalCity || originalCity.isAlive === false) return
