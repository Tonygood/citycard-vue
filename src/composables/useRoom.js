...
export function useRoom() {
  ...
  async function saveRoomData(id, data) {
    ...
    debugLog('[Firebase] 准备保存到Firebase的数据:', {
      roomId: id,
    ...
    debugLog('[Firebase] 完整gameState对象:', JSON.stringify(updatedData.gameState, null, 2))
    debugLog('[Firebase] ===== knownCities 详细诊断 =====')
    debugLog('[Firebase] knownCities存在:', !!updatedData.gameState?.knownCities)
    debugLog('[Firebase] knownCities内容:', JSON.stringify(updatedData.gameState?.knownCities, null, 2))

    if (isFirebaseReady()) {
      // 使用 Firebase
      debugLog(`[Firebase] 正在保存房间数据到 Firebase: ${id}`)
      try {
        const db = getDatabase()
        await set(dbRef(db, 'rooms/' + id), updatedData)
        debugLog(`[Firebase] ✓ 房间数据保存成功: ${id}`)
        debugLog('[Firebase] ✓ 保存后验证gameState:', updatedData.gameState ? '存在' : '不存在')

        // 关键诊断：立即读取验证knownCities是否真的保存到Firebase
        debugLog('[Firebase] ===== 保存后立即验证 =====')
        const verifySnapshot = await get(dbRef(db, 'rooms/' + id + '/gameState/knownCities'))
        const savedKnownCities = verifySnapshot.val()
        debugLog('[Firebase] 验证读取knownCities:', savedKnownCities ? JSON.stringify(savedKnownCities, null, 2) : 'null/undefined')
        debugLog('[Firebase] ========================================')
      ...
      // 使用 localStorage
      debugLog(`[localStorage] 正在保存房间数据到本地存储: ${id}`)
  ...
  async function updateRoomGameState(id, updates) {
    debugLog('[Firebase] 准备部分更新gameState:', {
      roomId: id,
    ...
    if (isFirebaseReady()) {
      try {
        ...
        if (!roomData.gameState) {
          debugLog('[Firebase] gameState不存在（旧房间），先初始化')
          // 合并初始化和更新操作，减少写入次数
          ...
          debugLog(`[Firebase] ✓ gameState初始化并更新成功: ${id}`, Object.keys(updates))
          return true
        }
        ...
        debugLog(`[Firebase] ✓ gameState部分更新成功: ${id}`, Object.keys(updates))
        return true
      } catch (error) {
  ...
  async function getRoomData(id) {
    ...
    if (isFirebaseReady()) {
      // 使用 Firebase
      debugLog(`[Firebase] 正在从 Firebase 读取房间数据: ${id}`)
      try {
        const db = getDatabase()
        const snapshot = await get(dbRef(db, 'rooms/' + id))
        data = snapshot.val()
        if (data) {
          debugLog(`[Firebase] ✓ 房间数据读取成功: ${id}`)
        } else {
          debugLog(`[Firebase] ⚠ 房间不存在: ${id}`)
          return null
        }
      } catch (error) {
      ...
      // 使用 localStorage
      debugLog(`[localStorage] 正在从本地存储读取房间数据: ${id}`)
    ...
    if (data && data.lastActivity) {
      ...
      if (timeSinceActivity > ROOM_EXPIRY_MS) {
        debugLog(`[房间过期] 房间 ${id} 已过期（${Math.floor(timeSinceActivity / 3600000)}小时无活动）`)
  ...
  function listenToRoomData(id, callback) {
    if (isFirebaseReady()) {
      // 使用 Firebase 实时监听
      debugLog(`[Firebase] 开始监听房间数据变化: ${id}`)
      const db = getDatabase()
      const roomRef = dbRef(db, 'rooms/' + id)
      onValue(roomRef, (snapshot) => {
        const data = snapshot.val()
        if (data) {
          debugLog(`[Firebase] 收到房间数据更新: ${id}`, {
            ...
            hasPendingIntercept: !!data.gameState?.pendingIntercept,
            pendingInterceptStatus: data.gameState?.pendingIntercept?.status || 'none',
            timestamp: Date.now()
          })
  ...
  async function sendHeartbeat(id, playerName) {
    try {
      if (isFirebaseReady()) {
        ...
        debugLog(`[心跳] 已更新玩家 ${playerName} 的心跳状态（轻量级）`)
        return true
      } else {
        // localStorage：需要读取完整数据
        const data = await getRoomData(id)
        if (!data) {
          debugLog(`[心跳] 房间 ${id} 不存在或已过期`)
          return false
        }
        ...
        debugLog(`[心跳] 已更新玩家 ${playerName} 的心跳状态`)
        return true
      }
    } catch (error) {
  ...
  function startHeartbeat(id, playerName) {
    ...
    debugLog(`[心跳] 开始心跳检查 - 房间: ${id}, 玩家: ${playerName}`)
  ...
  async function joinRoom(id) {
    ...
    debugLog(`[加入房间] 房间玩家: ${data.players.length}, 在线玩家: ${onlinePlayers.length}, 房间容量: ${data.playerCount}, 是否满员: ${isRoomFull}`)
  ...
  async function kickOfflinePlayer(id, playerName) {
    try {
      ...
      if (playerIndex >= 0) {
        ...
        debugLog(`[踢人] 已从玩家列表移除: ${playerName}`)
      }
      ...
      if (data.spectators) {
        ...
        if (spectatorIndex >= 0) {
          ...
          debugLog(`[踢人] 已从围观者列表移除: ${playerName}`)
        }
      }
      ...
      if (data.heartbeats && data.heartbeats[playerName]) {
        ...
        debugLog(`[踢人] 已清理心跳记录: ${playerName}`)
      }
  ...
  async function addPlayerToRoom(id, player, asSpectator = false) {
    ...
    if (asSpectator) {
      ...
      if (existingPlayerIndex >= 0) {
        ...
        debugLog('[加入房间] 检测到断线重连，使用partial update更新心跳')
        if (isFirebaseReady()) {
          const db = getDatabase()
          const updates = {}
        ...
        // 关键修复：再次读取最新数据防止竞态条件（两个玩家同时加入）
        const freshData = await getRoomData(id)
        if (freshData && freshData.players) {
          const alreadyJoined = freshData.players.find(p => p.name === player.name)
          if (alreadyJoined) {
            // 玩家已被另一个并发请求添加
            debugLog('[加入房间] 玩家已存在（竞态条件），跳过重复添加')
            startHeartbeat(id, player.name)
            return { success: true, roomData: freshData }
          }
          if (freshData.players.length >= (freshData.playerCount || data.playerCount)) {
            return { success: false, error: '房间已满！' }
          }
          // 使用最新数据
          data.players = freshData.players
        }
        // 新玩家
        data.players.push({
          name: player.name,
      ...
      try {
        ...
        if (data) {
          ...
          if (data.players && Array.isArray(data.players)) {
            ...
            if (playerIndex >= 0) {
              data.players.splice(playerIndex, 1)
              debugLog(`[退出] 已从玩家列表移除: ${playerName}`)
            }
          }
          ...
          if (data.spectators && Array.isArray(data.spectators)) {
            ...
            if (spectatorIndex >= 0) {
              data.spectators.splice(spectatorIndex, 1)
              debugLog(`[退出] 已从围观者列表移除: ${playerName}`)
            }
          }
          ...
          if (data.heartbeats && data.heartbeats[playerName]) {
            delete data.heartbeats[playerName]
            debugLog(`[退出] 已清理心跳记录: ${playerName}`)
          }

          await saveRoomData(id, data)
          debugLog(`[退出] 玩家 ${playerName} 已完全从房间移除`)
        }
      } catch (error) {
