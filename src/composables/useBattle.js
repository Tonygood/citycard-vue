...
export function useBattle() {
  ...
  function calculateBattle(attacker, defender) {
    ...
    if (gameStore.barrier[defender.name]) {
      ...
      debugLog(`[useBattle] 屏障吸收伤害: ${Math.floor(absorbed)}, 剩余HP: ${Math.floor(barrier.hp)}`)
  ...
  function updateBarriers() {
    ...
    // 这里保留空函数以保持向后兼容
    debugLog('[useBattle] updateBarriers 已废弃，屏障更新由 gameStore.updateRoundStates() 处理')
  }
  ...
  function battleBetweenCities(player1, city1Name, player2, city2Name) {
    ...
    if (!city1.isAlive || !city2.isAlive) {
      debugLog('[useBattle] 城市已阵亡，无法战斗', { city1Name, city2Name })
      return null
    }
