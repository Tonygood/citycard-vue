...
export function useGameLogic() {
  ...
  function battle2P(players, gameState) {
    ...
    // 注意：同省撤退/归顺会在preBattleChecks中清空currentBattleCities，
    // 需要检查specialEventThisRound避免误判"未出牌"
    const hasSpecialEvent = gameState.specialEventThisRound &&
      (gameState.specialEventThisRound.type === 'retreat' || gameState.specialEventThisRound.type === 'surrender')
    ;[
      ...
      if (casterState.battleGoldSkill === '草木皆兵') {
        ...
        if (targetOut.length === 0 && !hasSpecialEvent) {
        ...
        } else if (targetOut.length === 0 && hasSpecialEvent) {
          console.log(`[battle2P] 草木皆兵跳过：本轮发生了${gameState.specialEventThisRound.type}事件，不判定未出牌`)
        }
      }
    })
