...
export const useGameStore = defineStore('game', () => {
  ...
  const revivedCities = reactive({})          // 本轮复活的城市 [player][cityName]: true（每回合清空）
  ...
  const cityTrialField = reactive({})         // 城市试炼场 [player][cityName]: {originalHp, roundsLeft, startRound}
  ...
  const hjbf = reactive({})                   // 厚积薄发 [player][cityName]: {roundsLeft, accumulatedPower}
  const botaoxiongyong = reactive({})         // 波涛汹涌 [target]: {active, roundsLeft}
  ...
  const mutualWatch = reactive({})            // 守望相助 [player][cityName]: {province, cityName} (城市阵亡时召唤同省城市)
  const actingCapital = reactive({})          // 代行省权 [player]: {cityName, province} (代行省权状态)
  const forcedBench = reactive({})             // 避而不见禁用城市 [player][cityName]: rounds

  // ========== 特殊事件 ==========
  const specialEventThisRound = ref(null)      // 本回合特殊事件 {type, round, ...} (retreat/surrender)
  ...
  // ========== 无懈可击拦截系统 ==========
  const pendingIntercept = ref(null)          // {id, casterName, targetName, skillName, skillData, skillCost, interceptCost, status, createdAt, expiresAt, round}
  ...
  // ========== 单回合技能花费追踪 ==========
  const roundGoldSpent = reactive({})         // 单回合金币花费 [player]: { round, total }
  ...
  function resetGame() {
    ...
    Object.keys(revivedCities).forEach(key => delete revivedCities[key])
    ...
    Object.keys(mutualWatch).forEach(key => delete mutualWatch[key])
    Object.keys(actingCapital).forEach(key => delete actingCapital[key])
    Object.keys(forcedBench).forEach(key => delete forcedBench[key])
    specialEventThisRound.value = null
    ...
    Object.keys(cityTrialField).forEach(key => delete cityTrialField[key])
    Object.keys(hjbf).forEach(key => delete hjbf[key])
    Object.keys(botaoxiongyong).forEach(key => delete botaoxiongyong[key])
    Object.keys(cautionSet).forEach(key => delete cautionSet[key])
    Object.keys(gaixiangengzhangUsed).forEach(key => delete gaixiangengzhangUsed[key])
  ...
  function isInCautiousSet(playerName, cityName) {
    ...
    if (player && player.centerCityName === cityName) {
      return true
    }
    // 检查钢铁城市
    if (ironCities[playerName] && ironCities[playerName][cityName] > 0) {
      return true
    }
    // 检查定海神针保护（anchored）
    if (anchored[playerName] && anchored[playerName][cityName]) {
      return true
    }
    ...
    // 检查副中心城市
    if (subCenters[playerName] === cityName) {
      return true
    }
    // 检查生于紫室城市
    if (purpleChamber[playerName] === cityName) {
      return true
    }
    return false
  }
  ...
  function checkCenterDeathAndPurpleChamberInheritance(player) {
    ...
    // 中心城市未阵亡，无需处理（兼容 currentHp 和 hp 两种字段）
    const centerHp = centerCity ? (centerCity.currentHp !== undefined ? centerCity.currentHp : (centerCity.hp || 0)) : 0
    if (!centerCity || centerHp > 0) {
      return
    }
    ...
    const chamberHp = chamberCity ? (chamberCity.currentHp !== undefined ? chamberCity.currentHp : (chamberCity.hp || 0)) : 0
    const hasPurpleChamber = chamberCity && chamberHp > 0 && chamberCity.isAlive !== false
  ...
  /**
   * 创建待拦截请求
   * @param {Object} interceptData - 拦截数据
   * @returns {Object} 创建的pendingIntercept对象
   */
  function createPendingIntercept(interceptData) {
    const id = `intercept_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    const now = Date.now()
    const data = {
      id,
      casterName: interceptData.casterName,
      targetName: interceptData.targetName,
      skillName: interceptData.skillName,
      skillData: interceptData.skillData || {},
      skillCost: interceptData.skillCost,
      interceptCost: interceptData.interceptCost,
      status: 'pending', // 'pending' | 'accepted' | 'countered' | 'expired'
      createdAt: now,
      expiresAt: now + 45000, // 45秒超时
      round: currentRound.value
    }
    pendingIntercept.value = data
    // 返回纯对象副本（非Vue Proxy），确保外部使用时不受反应性包装影响
   ...
   */
  function updatePendingInterceptStatus(newStatus) {
    if (pendingIntercept.value) {
      pendingIntercept.value = { ...pendingIntercept.value, status: newStatus }
    }
  }
   ...
   * @returns {Object} 纯对象副本
   */
  function createPendingFortuneSwap(data) {
    const id = `fortune_swap_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    const swapData = {
      id,
      type: data.type, // '时来运转' | '人质交换'
      casterName: data.casterName,
      targetName: data.targetName,
      casterCities: data.casterCities || [],
      targetCities: data.targetCities || [],
      status: 'pending', // 'pending' | 'accepted' | 'li_dai_tao_jiang' | 'countered'
      selectedCities: null,
      createdAt: Date.now(),
      round: currentRound.value
    }
    pendingFortuneSwap.value = swapData
    return JSON.parse(JSON.stringify(swapData))
  }

  /**
   * 更新待处理的时来运转/人质交换请求状态
   * @param {string} status - 新状态
   * @param {string[]|null} selectedCities - 李代桃僵选择的城市
   */
  function updatePendingFortuneSwapStatus(status, selectedCities = null) {
    if (pendingFortuneSwap.value) {
      pendingFortuneSwap.value = {
        ...pendingFortuneSwap.value,
        status,
        selectedCities
      }
   ...
   * 清除待处理的时来运转/人质交换请求
   */
  function clearPendingFortuneSwap() {
    pendingFortuneSwap.value = null
  }
  ...
  function updateRoundStates() {
    debugLog('[gameStore.updateRoundStates] ===== 函数被调用 =====')
    debugLog('[gameStore.updateRoundStates] 当前回合:', currentRound.value)
    debugLog('[gameStore.updateRoundStates] goldLoanRounds状态:', JSON.stringify(goldLoanRounds))
    debugLog('[gameStore.updateRoundStates] 玩家列表:', players.value.map(p => ({ name: p.name, gold: p.gold })))

    // 更新金币贷款禁用
    // 注意：在线模式和单机模式都使用这个函数
    debugLog('[gameStore.updateRoundStates] 开始处理金币贷款扣除')
    debugLog('[gameStore.updateRoundStates] goldLoanRounds keys:', Object.keys(goldLoanRounds))

    for (const playerName of Object.keys(goldLoanRounds)) {
      debugLog(`[gameStore.updateRoundStates] 检查玩家 ${playerName} 的金币贷款状态`)
      debugLog(`[gameStore.updateRoundStates] ${playerName} 的 goldLoanRounds 值:`, goldLoanRounds[playerName])

      if (goldLoanRounds[playerName] > 0) {
        debugLog(`[gameStore.updateRoundStates] ${playerName} 的金币贷款冷却中，剩余 ${goldLoanRounds[playerName]} 回合`)

        // 找到对应的玩家对象
        const player = players.value.find(p => p.name === playerName)
        debugLog(`[gameStore.updateRoundStates] 找到玩家对象:`, player ? `${player.name}, 当前金币: ${player.gold}` : '未找到')
        ...
        if (player) {
          ...
          debugLog(`[gameStore.updateRoundStates] ${playerName} 金币扣除: ${goldBefore} -> ${player.gold}`)
          addLog(`${playerName} 金币贷款冷却中(剩余${goldLoanRounds[playerName]}回合)，扣除3金币`)
        }
        // 递减冷却回合数
        goldLoanRounds[playerName]--
        debugLog(`[gameStore.updateRoundStates] ${playerName} 的 goldLoanRounds 递减后:`, goldLoanRounds[playerName])
      }
    }

    debugLog('[gameStore.updateRoundStates] 金币贷款扣除处理完成')
    ...
    // 更新定海神针保护回合数
    for (const player of Object.keys(anchored)) {
      for (const cityName of Object.keys(anchored[player])) {
        if (anchored[player][cityName] > 0) {
          anchored[player][cityName]--
          if (anchored[player][cityName] <= 0) {
            delete anchored[player][cityName]
            addLog(`${player}的${cityName}定海神针效果消失`)
          }
        }
      }
      if (Object.keys(anchored[player]).length === 0) {
        delete anchored[player]
      }
    }
    ...
    for (const player of Object.keys(purpleChamber)) {
      ...
      // 动态HP上限：baseHp<=30000时上限80000，baseHp>30000时上限100000
      const hpCap = baseHp > 30000 ? 100000 : 80000
      city.currentHp = Math.min(city.currentHp + increaseAmount, hpCap)
      ...
      for (const cityName of Object.keys(hiddenGrowth[player])) {
        ...
        if (didFight) {
          ...
          if (cfg.idleRounds % 5 === 0) {
            const before = city.currentHp
            // 动态HP上限：baseHp<=30000时上限80000，baseHp>30000时上限100000
            const baseHpForCap = initialCities[player] && initialCities[player][city.name]
              ? initialCities[player][city.name].hp
              : city.hp
            const hiddenHpCap = baseHpForCap > 30000 ? 100000 : 80000
            city.currentHp = Math.min(city.currentHp + 10000, hiddenHpCap)
  ...
  // 记录单回合技能金币花费
  function addRoundGoldSpent(playerName, cost) {
    const round = currentRound.value
    if (!roundGoldSpent[playerName] || roundGoldSpent[playerName].round !== round) {
      roundGoldSpent[playerName] = { round, total: 0 }
    }
    roundGoldSpent[playerName].total += cost
  }

  // 获取当前回合技能金币花费
  function getRoundGoldSpent(playerName) {
    if (!roundGoldSpent[playerName]) return 0
    if (roundGoldSpent[playerName].round !== currentRound.value) return 0
    return roundGoldSpent[playerName].total
  }
  ...
  return {
    ...
    updatePendingInterceptStatus,
    clearPendingIntercept,

    // ========== 时来运转/人质交换弹窗系统方法 ==========
    createPendingFortuneSwap,
    updatePendingFortuneSwapStatus,
    clearPendingFortuneSwap,
